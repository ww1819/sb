...
public class PmDeviceController {
    ...
    public Result<PageResult<Map<String, Object>>> page(
            PageQuery query,
            @RequestParam(required = false) Boolean pmOnly) {
        StringBuilder where = new StringBuilder(" WHERE d.is_active = true ");
        List<Object> args = new ArrayList<>();
        if (Boolean.TRUE.equals(pmOnly)) {
            where.append(" AND d.is_pm_device = true ");
        }
        ...
        var rows = jdbc.queryForList("""
                SELECT d.id, d.device_code, d.device_name, d.specification, d.is_pm_device,
                ...
                LEFT JOIN pm_plan p ON p.device_id = d.id AND p.status = 'active'
    ...
    @PostMapping("/toggle")
    @Transactional
    @OperationLog(module = "pm", description = "切换预防性维护设备标记")
    public Result<Void> toggle(@RequestBody Map<String, Object> body) {
        ...
        boolean flag = Boolean.TRUE.equals(body.get("is_pm_device"));
        for (String deviceId : deviceIds) {
            jdbc.update("UPDATE medical_device SET is_pm_device=?, updated_at=NOW() WHERE id=?::uuid", flag, deviceId);
        }
    ...
    @Transactional
    @OperationLog(module = "pm", description = "批量生成预防性维护执行单")
    ...
    private Map<String, Object> generateOneExecution(UUID planId, Map<String, Object> body) {
        var plan = jdbc.queryForList("""
                SELECT p.*, t.template_name, t.pm_type_id
                FROM pm_plan p
        ...
        String execNo = PmExecutionController.nextNo();
        jdbc.update("""
        ...
        jdbc.update("""
                INSERT INTO pm_execution_item (id, execution_id, device_id, device_code, device_name, dept_id, plan_id, status)
        ...
        var templateItems = jdbc.queryForList("""
                SELECT * FROM pm_template_item WHERE template_id = ?::uuid ORDER BY sort_order, created_at
                """, p.get("template_id"));
        if (templateItems.isEmpty() && p.get("template_id") != null) {
            templateItems = jdbc.queryForList(
                    "SELECT id AS template_item_id, item_name, item_content FROM pm_template_item WHERE template_id=?::uuid",
                    p.get("template_id"));
        }
        for (Map<String, Object> ti : templateItems) {
            jdbc.update("""
                    INSERT INTO pm_execution_result (id, execution_item_id, template_item_id, item_name, item_content, result_status)
                    VALUES (?::uuid,?::uuid,?::uuid,?,?,?)
                    """, UUID.randomUUID(), itemId, ti.get("id"), ti.get("item_name"), ti.get("item_content"), "pending");
        }
        return jdbc.queryForList("SELECT * FROM pm_execution WHERE id=?::uuid", execId).get(0);
    }
}
