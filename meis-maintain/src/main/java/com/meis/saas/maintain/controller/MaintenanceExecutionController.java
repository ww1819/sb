...
public class MaintenanceExecutionController {
    ...
    public Result<PageResult<Map<String, Object>>> page(
        ...
        var rows = jdbc.queryForList("""
                SELECT e.*, COALESCE(e.template_name, t.template_name) AS template_name,
                       ...
                       EXISTS (
                         SELECT 1 FROM maintenance_execution_item ei
    ...
    public Result<Map<String, Object>> updateHeader(@PathVariable UUID id, @RequestBody Map<String, Object> body) {
        ...
        SoftDeleteSupport.appendUpdateChannel(jdbc, "maintenance_execution", sets, args, client);
    ...
    public Result<Map<String, Object>> patchItem(@PathVariable UUID itemId, @RequestBody Map<String, Object> body) {
        ...
        SoftDeleteSupport.appendUpdateChannel(jdbc, "maintenance_execution_item", sets, args, client);
    ...
    @DeleteMapping("/{id}")
    @Transactional
    @OperationLog(module = "maintain", description = "删除保养执行单")
    public Result<Void> deleteExecution(
        ...
        Map<String, Object> payload = body != null ? new LinkedHashMap<>(body) : new LinkedHashMap<>();
        if (client != null && !payload.containsKey("client")) payload.put("client", client);
        OpsExecutionItemSupport.deleteExecution(jdbc, docLog, "maintain",
                "maintenance_execution", "maintenance_execution_item",
    ...
    private void touchItemUpdateChannel(UUID itemId, String client) {
        ...
        if (sets.isEmpty()) return;
        args.add(itemId);
        jdbc.update("UPDATE maintenance_execution_item SET " + String.join(", ", sets) + " WHERE id=?::uuid", args.toArray());
    }
