...
public class PmPlanController {
    ...
    private final PmDeviceController deviceController;
    ...
    public Result<Map<String, Object>> save(@RequestBody Map<String, Object> body) {
        ...
        boolean exists = !jdbc.queryForList("SELECT 1 FROM pm_plan WHERE id = ?::uuid", id).isEmpty();
        if (exists) {
            jdbc.update("""
                UPDATE pm_plan SET plan_name=?, template_id=?::uuid, device_id=?::uuid, dept_id=?::uuid,
                pm_type=?, cycle_type=?, cycle_value=?, cycle_days=?, next_due_date=?, last_maintained_at=?,
                status=?, approval_status=?, remark=?, updated_at=NOW()
                WHERE id=?::uuid
                """, body.get("plan_name"), body.get("template_id"), body.get("device_id"), body.get("dept_id"),
                    body.get("pm_type"), body.get("cycle_type"), body.get("cycle_value"),
            ...
            jdbc.update("""
                INSERT INTO pm_plan (id, plan_code, plan_name, template_id, device_id, dept_id, pm_type,
                cycle_type, cycle_value, cycle_days, next_due_date, status, approval_status, created_by, remark)
                VALUES (?::uuid,?,?,?::uuid,?::uuid,?::uuid,?,?,?,?,?,?,?,?,?)
                """, id, body.getOrDefault("plan_code", "PP" + System.currentTimeMillis()), body.get("plan_name"),
                    body.get("template_id"), body.get("device_id"), body.get("dept_id"),
                    body.getOrDefault("pm_type", "L1"), body.get("cycle_type"), body.get("cycle_value"),
    ...
    public Result<Map<String, Object>> approve(@PathVariable UUID id, @RequestBody Map<String, Object> body) {
        ...
        jdbc.update("""
                UPDATE pm_plan SET approval_status=?, approved_by=?::uuid, approved_at=NOW(),
    ...
    @Transactional
    @OperationLog(module = "pm", description = "从模板生成预防性维护计划")
    public Result<List<Map<String, Object>>> generate(@RequestBody Map<String, Object> body) {
        ...
        var template = jdbc.queryForList("SELECT * FROM pm_template WHERE id = ?::uuid", templateId);
        ...
        for (String deviceId : deviceIds) {
            ...
            jdbc.update("""
                INSERT INTO pm_plan (id, plan_code, plan_name, template_id, device_id, pm_type,
                    cycle_type, cycle_value, cycle_days, next_due_date, status, approval_status, created_by)
                VALUES (?::uuid,?,?,?::uuid,?::uuid,?,?,?,?,?,?,?,?)
                """, id, "PP" + System.nanoTime(), template.get(0).get("template_name") + "计划",
                    templateId, deviceId, template.get(0).get("pm_type"),
            ...
            created.add(jdbc.queryForList("SELECT * FROM pm_plan WHERE id = ?::uuid", id).get(0));
        }
