...
public class MaintenancePlanController {
    ...
    @Transactional
    @OperationLog(module = "maintain", description = "申请纳入保养计划")
    ...
    public Result<List<Map<String, Object>>> listIncludeRequests(@PathVariable UUID id) {
        return Result.ok(OpsPlanIncludeSupport.listByPlan(jdbc, "maintain", id));
    }

    @PostMapping("/include-request/{requestId}/approve")
    @Transactional
    @OperationLog(module = "maintain", description = "确认纳入保养计划")
    ...
    @Transactional
    @OperationLog(module = "maintain", description = "驳回纳入保养计划")
    ...
    public Result<Map<String, Object>> get(@PathVariable UUID id) {
        var rows = jdbc.queryForList("""
                SELECT p.*, t.template_name AS join_template_name, l.level_name AS maintenance_level_name
                FROM maintenance_plan p
                LEFT JOIN maintenance_template t ON t.id = p.template_id
                LEFT JOIN maintenance_level l ON l.id = p.maintenance_level_id
                WHERE p.id = ?::uuid
    ...
    public Result<Map<String, Object>> save(@RequestBody Map<String, Object> body) {
        ...
        if (exists) {
            var old = jdbc.queryForList(
                    "SELECT plan_no, plan_code, approval_status FROM maintenance_plan WHERE id = ?::uuid", id);
            ...
            if (planNo == null || planNo.isBlank()) {
                planNo = DailyBizNoSupport.next(jdbc, "maintenance_plan", "plan_no", "MP-");
            }
        } else {
            planNo = DailyBizNoSupport.next(jdbc, "maintenance_plan", "plan_no", "MP-");
        }
        ...
        if ((templateName == null || templateName.isBlank()) && body.get("template_id") != null) {
            var t = jdbc.queryForList(
                    "SELECT template_name FROM maintenance_template WHERE id = ?::uuid"
                            + SoftDeleteSupport.notDeletedClause(jdbc, "maintenance_template", null),
        ...
        String assignedUserId = asBlankToNull(body.get("assigned_user_id"));
        String assignedUserName = asBlankToNull(body.get("assigned_user_name"));
        ...
        if (exists) {
            jdbc.update("""
                    UPDATE maintenance_plan SET plan_name=?, plan_no=?, plan_code=COALESCE(?, plan_code),
                    template_id=?::uuid, template_name=?, maintenance_level=?, maintenance_level_id=?::uuid,
                    ...
                    body.getOrDefault("maintenance_level", "L1"), blankToNull(body.get("maintenance_level_id")),
            ...
            int cycleDays = toPositiveInt(body.get("cycle_days"), 30);
            LocalDate nextDue = LocalDate.now().plusDays(cycleDays);
            jdbc.update("""
                    INSERT INTO maintenance_plan (id, plan_code, plan_no, plan_name, template_id, template_name,
                    maintenance_level, maintenance_level_id, cycle_type, cycle_value, cycle_days,
                    ...
                    body.getOrDefault("maintenance_level", "L1"), blankToNull(body.get("maintenance_level_id")),
                    body.getOrDefault("cycle_type", "day"), body.get("cycle_value"),
                    cycleDays, nextDue,
                    body.getOrDefault("reminder_days_before", 7),
                    body.getOrDefault("status", "active"), body.getOrDefault("approval_status", "draft"),
                    blankToNull(body.get("dept_id")), blankToNull(body.get("campus_id")),
                    assignedUserId, assignedUserName,
                    userId, createdByName, body.get("remark"));
        }

        upsertItems(id, planNo, itemsEarly, clientOf(body));
        refreshPlanDueCache(id);
        docLog.event("maintain", "plan", id, planNo, exists ? "update" : "create", clientOf(body), null);
        return get(id);
    }

    @GetMapping("/{id}/change-logs")
    public Result<List<Map<String, Object>>> changeLogs(@PathVariable UUID id) {
        return Result.ok(docLog.list("maintain", "plan", id));
    }
    ...
    public Result<Map<String, Object>> approve(@PathVariable UUID id, @RequestBody Map<String, Object> body) {
        ...
        jdbc.update("""
                UPDATE maintenance_plan SET approval_status=?, approved_by=?::uuid, approved_by_name=?,
                approved_at=NOW(),
                status=CASE WHEN ?='approved' THEN 'active' ELSE status END, updated_at=NOW()
                WHERE id=?::uuid
                """, status, userId, userName, status, id);
        var plan = get(id).getData();
        docLog.event("maintain", "plan", id, Objects.toString(plan.get("plan_no"), null),
    ...
    @Transactional
    @OperationLog(module = "maintain", description = "计划执行补录")
    ...
    @Transactional
    @OperationLog(module = "maintain", description = "从模板生成保养计划（一单多设备）")
    public Result<Map<String, Object>> generate(@RequestBody Map<String, Object> body) {
        ...
        String planNo = DailyBizNoSupport.next(jdbc, "maintenance_plan", "plan_no", "MP-");
        Map<String, Object> t = template.get(0);
        String userId = TenantContext.getUserId();
        String createdByName = SoftDeleteSupport.resolveUserDisplayName(jdbc, userId);
        CycleDaysSupport.Cycle cycle = CycleDaysSupport.resolveFromTemplate(body, t, "day", 1);
        ...
        jdbc.update("""
                INSERT INTO maintenance_plan (id, plan_code, plan_no, plan_name, template_id, template_name,
                maintenance_level, maintenance_level_id, cycle_type, cycle_value, cycle_days, next_due_date,
                status, approval_status, created_by, created_by_name)
                VALUES (?::uuid,?,?,?,?::uuid,?,?,?::uuid,?,?,?,?,?,?,?::uuid,?)
                """, id, planNo, planNo, t.get("template_name") + "计划", templateId, t.get("template_name"),
                t.getOrDefault("maintenance_level", "L1"), t.get("maintenance_level_id"),
        ...
        refreshPlanDueCache(id);
        docLog.event("maintain", "plan", id, planNo, "create", clientOf(body), "from_template");
        return get(id);
    }
    ...
    public Result<List<Map<String, Object>>> due() {
        return Result.ok(jdbc.queryForList("""
                SELECT i.*, p.plan_name, p.plan_no, p.template_id, p.maintenance_level, p.approval_status
                FROM maintenance_plan_item i
    ...
    private void upsertItems(UUID planId, String planNo, List<Map<String, Object>> items, String client) {
        ...
        for (Map<String, Object> row : jdbc.queryForList(
                "SELECT * FROM maintenance_plan_item WHERE plan_id=?::uuid", planId)) {
        ...
        for (Map<String, Object> item : items) {
            ...
            if (activeExists || restore) {
                ...
                docLog.fieldChange("maintain", "plan", planId, planNo, "item", itemId,
        ...
        for (Map.Entry<UUID, Map<String, Object>> e : existing.entrySet()) {
            if (!keep.contains(e.getKey())) {
                docLog.fieldChange("maintain", "plan", planId, planNo, "item", e.getKey(),
                        "device_code", e.getValue().get("device_code"), null, client);
            }
        }
        if (keep.isEmpty()) {
            jdbc.update("UPDATE maintenance_plan_item SET is_deleted=1, deleted_at=NOW(), updated_at=NOW() WHERE plan_id=?::uuid AND COALESCE(is_deleted,0)=0", planId);
        } else {
            String placeholders = String.join(",", Collections.nCopies(keep.size(), "?::uuid"));
            List<Object> args = new ArrayList<>();
            args.add(planId);
            args.addAll(keep);
            jdbc.update("UPDATE maintenance_plan_item SET is_deleted=1, deleted_at=NOW(), updated_at=NOW() "
    ...
    private int planCycleDays(UUID planId) {
        var rows = jdbc.queryForList("SELECT cycle_days FROM maintenance_plan WHERE id=?::uuid", planId);
        return toPositiveInt(rows.isEmpty() ? null : rows.get(0).get("cycle_days"), 30);
    }
    ...
    private void refreshPlanDueCache(UUID planId) {
        // 无明细或明细未填到期日时，保留计划头已有值，再按 cycle_days 回退，避免刷成 null 触发 NOT NULL
        jdbc.update("""
                UPDATE maintenance_plan SET
                  ...
                  last_maintained_at = (SELECT MAX(last_done_date) FROM maintenance_plan_item
    ...
    /** 前端日期多为 YYYY-MM-DD 字符串，空串转 null 供 ?::date 绑定 */
