...
@RestController
@RequestMapping("/api/maintain/query")
...
public class MaintenanceQueryController {
    ...
    public Result<PageResult<Map<String, Object>>> page(
        ...
        long total = jdbc.queryForObject("""
                SELECT COUNT(*) FROM maintenance_execution_item ei
        ...
        var rows = jdbc.queryForList("""
                SELECT ei.id, ei.device_code, ei.device_name, ei.overall_result, ei.updated_at AS completed_at,
                       ...
                       t.template_name, l.level_name AS maintenance_level_name, dept.dept_name
    ...
    public Result<Map<String, Object>> detail(@PathVariable UUID itemId) {
        var rows = jdbc.queryForList("""
                SELECT ei.*, e.execution_no, e.planned_date, t.template_name, l.level_name AS maintenance_level_name
                FROM maintenance_execution_item ei
                JOIN maintenance_execution e ON e.id = ei.execution_id
                LEFT JOIN maintenance_template t ON t.id = e.template_id
                LEFT JOIN maintenance_level l ON l.id = e.maintenance_level_id
