...
/**
 * 保养设备管理：台账过滤视图（附录 O.5 / OPS.4）——设备须出现在计划明细或执行明细中。
 */
@RestController
...
public class MaintenanceDeviceController {
    ...
    public Result<PageResult<Map<String, Object>>> page(PageQuery query) {
        ...
        String pi = SoftDeleteSupport.notDeletedClause(jdbc, "maintenance_plan_item", "pi");
        String ei = SoftDeleteSupport.notDeletedClause(jdbc, "maintenance_execution_item", "ei");
        StringBuilder where = new StringBuilder(
                " WHERE ("
                + " EXISTS (SELECT 1 FROM maintenance_plan_item pi WHERE pi.device_id = d.id " + pi + ")"
                + " OR EXISTS (SELECT 1 FROM maintenance_execution_item ei WHERE ei.device_id = d.id " + ei + ")"
                + " )");
        where.append(md);
        ...
        var rows = jdbc.queryForList(
                ...
                + " d.is_maintain_device,"
                + " (SELECT COUNT(*) FROM maintenance_plan_item pi WHERE pi.device_id=d.id " + pi + ") AS plan_count,"
                + " (SELECT COUNT(*) FROM maintenance_execution_item ei WHERE ei.device_id=d.id " + ei + ") AS execution_item_count"
