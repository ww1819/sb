...
public class PmExecutionController {
    ...
    public Result<Map<String, Object>> get(@PathVariable UUID id) {
        ...
        for (Map<String, Object> item : items) {
            ...
            item.put("results", jdbc.queryForList(
                    "SELECT * FROM pm_execution_result WHERE execution_item_id = ?::uuid ORDER BY created_at",
                    itemId));
        }
    ...
    public Result<Map<String, Object>> completeItem(@PathVariable UUID itemId, @RequestBody Map<String, Object> body) {
        ...
        var execRows = jdbc.queryForList("SELECT execution_id FROM pm_execution_item WHERE id = ?::uuid", itemId);
        if (!execRows.isEmpty()) {
        ...
        UUID execId = (UUID) jdbc.queryForList("SELECT execution_id FROM pm_execution_item WHERE id=?::uuid", itemId)
    ...
    private void updatePlansAfterComplete(UUID execId) {
        ...
        for (Map<String, Object> row : planIds) {
            ...
            var plan = jdbc.queryForList("SELECT cycle_days, next_due_date FROM pm_plan WHERE id=?::uuid", planId);
