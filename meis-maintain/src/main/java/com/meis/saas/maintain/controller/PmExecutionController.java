...
public class PmExecutionController {
    ...
    public Result<PageResult<Map<String, Object>>> page(
        ...
        var rows = jdbc.queryForList("""
                SELECT e.*, t.template_name, l.type_name AS pm_type_name,
                       eng.engineer_name AS assigned_engineer_name
                FROM pm_execution e
                LEFT JOIN pm_template t ON t.id = e.template_id
                LEFT JOIN pm_type l ON l.id = e.pm_type_id
    ...
    public Result<Map<String, Object>> start(@PathVariable UUID id, @RequestBody(required = false) Map<String, Object> body) {
        ...
        jdbc.update("""
                UPDATE pm_execution SET status='in_progress', executor_id=?::uuid,
                execute_start_time=COALESCE(execute_start_time, NOW()), updated_at=NOW()
                WHERE id=?::uuid AND status IN ('pending','draft')
                """, executorId, id);
        jdbc.update("UPDATE pm_execution_item SET status='in_progress', updated_at=NOW() WHERE execution_id=?::uuid AND status='pending'", id);
        return get(id);
    }
    ...
    public Result<Map<String, Object>> completeItem(@PathVariable UUID itemId, @RequestBody Map<String, Object> body) {
        ...
        for (Map<String, Object> r : results) {
            if (r.get("id") != null) {
                jdbc.update("""
                    UPDATE pm_execution_result SET result_value=?, result_status=?, remark=?, updated_at=NOW()
                    WHERE id=?::uuid
                    """, r.get("result_value"), r.getOrDefault("result_status", "pass"), r.get("remark"), r.get("id"));
            }
        }
        jdbc.update("""
                UPDATE pm_execution_item SET status='completed', overall_result=?, remark=?, updated_at=NOW()
                WHERE id=?::uuid
                """, body.getOrDefault("overall_result", "pass"), body.get("remark"), itemId);
        var execRows = jdbc.queryForList(
                "SELECT execution_id FROM pm_execution_item WHERE id = ?::uuid "
        ...
        if (!execRows.isEmpty()) {
            ...
            long pending = jdbc.queryForObject(
                    "SELECT COUNT(*) FROM pm_execution_item WHERE execution_id=?::uuid AND status <> 'completed'"
                            + SoftDeleteSupport.notDeletedClause(jdbc, "pm_execution_item", null),
                    Long.class, execId);
            if (pending == 0) {
                jdbc.update("UPDATE pm_execution SET status='completed', execute_end_time=NOW(), updated_at=NOW() WHERE id=?::uuid", execId);
    ...
    private void updatePlansAfterComplete(UUID execId) {
        var planIds = jdbc.queryForList("""
                SELECT DISTINCT plan_id FROM pm_execution_item
        ...
        for (Map<String, Object> row : planIds) {
            ...
            var plan = jdbc.queryForList(
                    "SELECT cycle_days, next_due_date FROM pm_plan WHERE id=?::uuid "
            ...
            jdbc.update("""
                    UPDATE pm_plan SET last_maintained_at=CURRENT_DATE, next_due_date=?, status='active', updated_at=NOW()
                    WHERE id=?::uuid
                    """, next, planId);
        }
    }
