...
@RestController
@RequestMapping("/api/pm/execution")
@RequiredArgsConstructor
public class PmExecutionController {
    private final JdbcTemplate jdbc;
    private final PmExecutionGenerator generator;
    ...
    public Result<PageResult<Map<String, Object>>> page(
        ...
        long total = jdbc.queryForObject(
                "SELECT COUNT(*) FROM pm_execution e" + where, Long.class, args.toArray());
    ...
    public Result<Map<String, Object>> get(@PathVariable UUID id) {
        var rows = jdbc.queryForList("""
                ...
                LEFT JOIN pm_type pt ON pt.id = e.pm_type_id
                WHERE e.id = ?::uuid
                """ + SoftDeleteSupport.notDeletedClause(jdbc, "pm_execution", "e"), id);
        ...
        for (Map<String, Object> item : items) {
            ...
            item.put("results", jdbc.queryForList(
                    "SELECT * FROM pm_execution_result WHERE execution_item_id = ?::uuid "
                            + SoftDeleteSupport.notDeletedClause(jdbc, "pm_execution_result", null)
    ...
    @PostMapping("/ad-hoc")
    @Transactional
    @OperationLog(module = "pm", description = "无计划直开预防性维护执行单")
    ...
    @PutMapping("/{id}")
    @Transactional
    @OperationLog(module = "pm", description = "更新预防性维护执行单头表")
    public Result<Map<String, Object>> updateHeader(@PathVariable UUID id, @RequestBody Map<String, Object> body) {
        ...
        var old = jdbc.queryForList(
                "SELECT * FROM pm_execution WHERE id=?::uuid"
                        + SoftDeleteSupport.notDeletedClause(jdbc, "pm_execution", null), id);
        ...
        if (body.containsKey("execute_start_time")) {
            ...
            docLog.fieldChange("pm", "execution", id, docNo, "header", id,
    ...
    @Transactional
    @OperationLog(module = "pm", description = "开始预防性维护执行")
    public Result<Map<String, Object>> start(@PathVariable UUID id, @RequestBody(required = false) Map<String, Object> body) {
        ...
        jdbc.update("""
                UPDATE pm_execution SET status='in_progress', executor_id=?::uuid, executor_name=?,
                execute_start_time=COALESCE(execute_start_time, NOW()), updated_at=NOW()
                WHERE id=?::uuid AND status IN ('draft','pending','in_progress')
                """, userId, name, id);
        jdbc.update("""
                UPDATE pm_execution_item SET status='in_progress',
                ...
                """, userId, name, id);
        SoftDeleteSupport.applyChannels(jdbc, "pm_execution", id, body, "update_channel");
        docLog.event("pm", "execution", id, execNo(id), "start", clientOf(body), null);
        return get(id);
    }

    @PatchMapping("/item/{itemId}")
    @Transactional
    @OperationLog(module = "pm", description = "更新预防性维护执行明细")
    public Result<Map<String, Object>> patchItem(@PathVariable UUID itemId, @RequestBody Map<String, Object> body) {
        var rows = jdbc.queryForList(
                "SELECT * FROM pm_execution_item WHERE id=?::uuid"
                        + SoftDeleteSupport.notDeletedClause(jdbc, "pm_execution_item", null), itemId);
        ...
        if (body.containsKey("photos")) {
            ...
            docLog.fieldChange("pm", "execution", execId, execNo, "item", itemId,
    ...
    @Transactional
    @OperationLog(module = "pm", description = "更新预防性维护执行内容结果")
    public Result<Map<String, Object>> patchResult(@PathVariable UUID resultId, @RequestBody Map<String, Object> body) {
        var rows = jdbc.queryForList(
                "SELECT r.*, ei.execution_id, ei.execution_no FROM pm_execution_result r "
                        + "INNER JOIN pm_execution_item ei ON ei.id = r.execution_item_id "
                        + "WHERE r.id=?::uuid"
                        + SoftDeleteSupport.notDeletedClause(jdbc, "pm_execution_result", "r"), resultId);
        ...
        if (body.containsKey("photos")) {
            sets.add("photos = CAST(? AS jsonb)");
            args.add(OpsPhotosSupport.toJson(body.get("photos")));
            docLog.fieldChange("pm", "execution", execId, execNo, "result", resultId,
                    "photos", old.get("photos"), body.get("photos"), client);
        }
        if (sets.isEmpty()) throw new BizException(400, "无更新字段");
        sets.add("row_version = COALESCE(row_version,1) + 1");
    ...
    @Transactional
    @OperationLog(module = "pm", description = "完成预防性维护设备项")
    public Result<Map<String, Object>> completeItem(@PathVariable UUID itemId, @RequestBody Map<String, Object> body) {
        ...
        for (Map<String, Object> r : results) {
            if (r.get("id") != null) {
                if (r.containsKey("photos")) {
                    jdbc.update("""
                        UPDATE pm_execution_result SET result_value=?, result_status=?, remark=?,
        ...
        SoftDeleteSupport.applyChannels(jdbc, "pm_execution_item", itemId, body, "update_channel");
        SoftDeleteSupport.applyChannels(jdbc, "pm_execution", execId, body, "update_channel");
        docLog.event("pm", "execution", execId, execNo(execId), "complete_item", clientOf(body), itemId.toString());
        var failed = jdbc.queryForList("""
                SELECT item_name, result_value, remark FROM pm_execution_result
                WHERE execution_item_id=?::uuid AND result_status='fail'
                """ + SoftDeleteSupport.notDeletedClause(jdbc, "pm_execution_result", null), itemId);
        if (!failed.isEmpty()) {
            OpsAutoRepairSupport.tryCreateFromFail(jdbc, docLog, "pm", execId, execNo(execId),
    ...
    @Transactional
    @OperationLog(module = "pm", description = "确认预防性维护执行明细")
    public Result<Map<String, Object>> confirmItem(@PathVariable UUID itemId, @RequestBody(required = false) Map<String, Object> body) {
        Map<String, Object> item = OpsExecutionItemSupport.loadItem(jdbc, "pm_execution_item", itemId);
        UUID execId = (UUID) item.get("execution_id");
        OpsExecutionItemSupport.confirmItem(jdbc, docLog, "pm",
    ...
    @Transactional
    @OperationLog(module = "pm", description = "删除预防性维护执行明细")
    public Result<Map<String, Object>> deleteItem(
        ...
        OpsExecutionItemSupport.deleteItem(jdbc, docLog, "pm",
                "pm_execution", "pm_execution_item",
    ...
    @Transactional
    @OperationLog(module = "pm", description = "提交预防性维护执行单")
    ...
    @Transactional
    @OperationLog(module = "pm", description = "撤回预防性维护执行单")
    ...
    @Transactional
    @OperationLog(module = "pm", description = "审核预防性维护执行单")
    ...
    public Result<List<Map<String, Object>>> changeLogs(@PathVariable UUID id) {
        return Result.ok(jdbc.queryForList("""
                ...
                WHERE module='pm' AND doc_type='execution' AND doc_id=?::uuid
    ...
    private String statusOf(UUID id) {
        var rows = jdbc.queryForList(
                "SELECT status FROM pm_execution WHERE id=?::uuid"
                        + SoftDeleteSupport.notDeletedClause(jdbc, "pm_execution", null), id);
        if (rows.isEmpty()) throw new BizException(404, "not found");
    ...
    private String execNo(UUID id) {
        var rows = jdbc.queryForList("SELECT execution_no FROM pm_execution WHERE id=?::uuid", id);
    ...
    private void track(List<String> sets, List<Object> args, Map<String, Object> old,
        ...
        docLog.fieldChange("pm", "execution", docId, docNo, entityType, entityId,
