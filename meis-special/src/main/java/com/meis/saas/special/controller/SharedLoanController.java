...
public class SharedLoanController {
    ...
    public Result<PageResult<Map<String, Object>>> page(PageQuery query,
        ...
        return Result.ok(queryLoans(query, status, pendingOnly));
    }
    ...
    public Result<Map<String, Object>> save(@RequestBody Map<String, Object> body) {
        ...
        if (!exists) {
            jdbc.update("""
                INSERT INTO shared_device_loan (id, loan_no, device_id, shared_device_id, device_code, device_name,
                from_dept_id, to_dept_id, applicant_id, loan_start, loan_end, fee_standard, reason, status, approval_status)
                VALUES (?::uuid,?,?::uuid,?::uuid,?,?,?::uuid,?::uuid,?::uuid,?,?,?,?,?,?,?)
                """, id, body.getOrDefault("loan_no", "SL" + System.currentTimeMillis()),
                    body.get("device_id"), body.get("shared_device_id"), body.get("device_code"),
                    body.get("device_name"), body.get("from_dept_id"), body.get("to_dept_id"),
                    body.get("applicant_id"), body.get("loan_start"), body.get("loan_end"),
                    body.get("fee_standard"), body.get("reason"),
                    body.getOrDefault("status", "draft"), body.getOrDefault("approval_status", "draft"));
        } else {
            jdbc.update("""
                UPDATE shared_device_loan SET to_dept_id=?::uuid, loan_start=?, loan_end=?, fee_standard=?,
                reason=?, remark=?, updated_at=NOW() WHERE id=?::uuid AND status IN ('draft','pending')
                """, body.get("to_dept_id"), body.get("loan_start"), body.get("loan_end"),
                    body.get("fee_standard"), body.get("reason"), body.get("remark"), id);
        }
        return get(id);
    }
    ...
    public Result<Map<String, Object>> lend(@PathVariable UUID id) {
        ...
        if (loan.get("shared_device_id") != null) {
            jdbc.update("""
                UPDATE shared_device SET availability_status='on_loan', updated_at=NOW() WHERE id=?::uuid
                """, loan.get("shared_device_id"));
        }
        return get(id);
    }

    private PageResult<Map<String, Object>> queryLoans(PageQuery query, String status, Boolean pendingOnly) {
    ...
    private void fillLoanSnapshot(Map<String, Object> body) {
        if (body.get("shared_device_id") != null) {
            var sd = jdbc.queryForList("SELECT * FROM shared_device WHERE id = ?::uuid", body.get("shared_device_id"));
            if (!sd.isEmpty()) {
                if (body.get("device_id") == null) body.put("device_id", sd.get(0).get("device_id"));
                if (body.get("device_code") == null) body.put("device_code", sd.get(0).get("device_code"));
                if (body.get("device_name") == null) body.put("device_name", sd.get(0).get("device_name"));
                if (body.get("from_dept_id") == null) body.put("from_dept_id", sd.get(0).get("owner_dept_id"));
                if (body.get("fee_standard") == null) body.put("fee_standard", sd.get(0).get("fee_standard"));
            }
        }
    }
}
