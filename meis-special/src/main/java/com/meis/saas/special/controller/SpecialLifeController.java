...
public class SpecialLifeController {
    ...
    public Result<Map<String, Object>> get(@PathVariable UUID id) {
        var rows = jdbc.queryForList("SELECT * FROM life_support_device WHERE id = ?::uuid", id);
    ...
    public Result<Map<String, Object>> save(@RequestBody Map<String, Object> body) {
        ...
        boolean exists = !jdbc.queryForList("SELECT 1 FROM life_support_device WHERE id = ?::uuid", id).isEmpty();
        if (!exists) {
            jdbc.update("""
            ...
            if (body.get("device_id") != null) {
                jdbc.update("UPDATE medical_device SET is_life_support = true, updated_at = NOW() WHERE id = ?::uuid",
                        body.get("device_id"));
            }
        } else {
            jdbc.update("""
                ...
                """, body.get("criticality_level"), body.get("backup_required"), body.get("standby_status"),
                    ...
                    body.get("responsible_person_id"), body.get("remark"), id);
        }
        return get(id);
    }
