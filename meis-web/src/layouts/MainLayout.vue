...
function onMenuSelect(path: string) {
  ...
  openPage(path, resolveTabTitle(path))
...
async function loadMenus() {
  ...
  try {
    ...
    if (!user?.tenantId || !user.schemaName || !user.userId) {
      menuError.value = '用户信息不完整，请重新登录'
      return
    }

    const { data } = await http.get('/system/menus/effective', {
      params: {
...
watch(titleMap, () => {
  ...
  for (const tab of tabsStore.tabs) {
    ...
    const title = resolveTabTitle(tab.path)
...
function buildTitleMap(list: NavModule[]) {
  ...
  map.set('/analytics/depr-stats', '资产折旧统计')
...
function resolveTabTitle(path: string) {
  ...
  return titleMap.value.get(path) ?? getPageConfig(path)?.title ?? defaultTitle(path)
