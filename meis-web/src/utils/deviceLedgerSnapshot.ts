...
export function deviceLedgerSnapshot(device: Record<string, unknown>) {
  return {
    ...
    acceptance_date: device.acceptance_date ?? null,
    enable_date: device.enable_date ?? null,
    ...
    install_location: device.install_location ?? null,
    location_detail: device.location_detail ?? null,
    responsible_person_name: device.responsible_person_name ?? device.use_dept_head ?? null,
