...
async function approve(form: Record<string, unknown>, reload?: () => void) {
  await http.post(`/inspect/plan/${form.id}/approve`, { action: 'approve', approved_by: auth.user?.userId })
  ElMessage.success('审核通过')
  reload?.()
}

async function reject(form: Record<string, unknown>, reload?: () => void) {
  await http.post(`/inspect/plan/${form.id}/approve`, { action: 'reject', approved_by: auth.user?.userId })
