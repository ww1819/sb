<template>
  <div class="life-support-page">
    <CrudPage ref="crudRef" :config="pageConfig" detail-mode @detail="openDetail" @add="openCreate">
      <template #toolbar-extra>
        <el-checkbox v-model="expiringOnly" @change="reload">仅显示30天内待测</el-checkbox>
      </template>
...
const baseConfig = getPageConfig('/special/life')!
...
const modalTitle = computed(() => (record.value?.id ? '编辑生命支持设备' : '登记生命支持设备'))
const formFields = getSchema('life_support_device')

function openCreate() {
  record.value = { criticality_level: 'high', backup_required: true, standby_status: 'ready' }
  visible.value = true
}

async function openDetail(row: Record<string, unknown>) {
  const { data } = await http.get(`/special/life/${row.id}`)
...
async function save() {
  ...
  const { data } = await http.post('/special/life', record.value)
  record.value = data.data
  visible.value = false
