    ...
    <el-form label-width="120px">
      ...
      <el-form-item label="下次执行日期">
        <div class="next-due-row">
          <el-checkbox v-model="form.update_next_due">修改</el-checkbox>
          <el-date-picker
...
function open(opts: {
  ...
  const t = `${d}T${pad(now.getHours())}:${pad(now.getMinutes())}:${pad(now.getSeconds())}`
  form.planned_date = d
  form.execute_start_time = t
  form.execute_end_time = t
