<template>
  <div class="finance-category-page">
    <aside class="tree-pane">
      <div class="tree-pane__header">财务分类树</div>
      <el-input
        v-model="treeKeyword"
        clearable
        placeholder="筛选分类"
...
async function loadTree() {
  ...
  try {
    const { data } = await http.get('/system/finance_category/list', { params: { limit: 500 } })
...
function buildTree(parentId: string | null): TreeNode[] {
  return flatRows.value
    ...
    .map((r) => {
      ...
      const node: TreeNode = {
        id,
        label: `${r.finance_code ?? ''} ${r.finance_name ?? ''}`.trim(),
        parentId
      }
      // 无子节点不挂空 children，避免叶子显示可展开箭头却展开为空
