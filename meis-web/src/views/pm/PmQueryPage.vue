  ...
  <div class="pm-query-page">
    <SystemPageCard title="预防性维护记录" :loading="loading" show-pager v-model:page="page" v-model:size="size" :total="total" @page-change="load">
      <template #filterBar>
        <PageFilterBar v-model:keyword="keyword" placeholder="设备名称 / 执行单号" @search="onSearch" @reset="onReset">
          <template #filters>
            ...
            <el-select v-model="resultStatus" placeholder="维护结果" clearable class="filter-item" @change="onSearch">
      ...
      <el-table v-loading="loading" :data="rows" stripe class="system-table" row-key="id" @row-dblclick="openDetail">
        ...
        <el-table-column prop="template_name" label="PM模板" min-width="140" />
        <el-table-column prop="pm_type_name" label="PM类型" width="100" />
        <el-table-column prop="overall_result" label="结果" width="80" />
        <el-table-column prop="completed_at" label="完成时间" min-width="160" />
      </el-table>
    </SystemPageCard>
    <AppModal v-model="visible" title="PM记录详情" size="lg">
      <template v-if="detail">
        ...
        <el-table :data="results" border size="small" class="result-table">
          <el-table-column prop="item_name" label="维护项目" />
          <el-table-column prop="item_content" label="维护内容" show-overflow-tooltip />
...
async function load() {
  loading.value = true
  try {
    const { data } = await http.get('/pm/query/page', {
...
async function openDetail(row: Record<string, unknown>) {
  const { data } = await http.get(`/pm/query/${row.id}`)
