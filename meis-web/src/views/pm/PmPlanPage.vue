<template>
  <WorkflowCrudPage :config="config" save-url="/pm/plan">
...
const config = getPageConfig('/pm/plan')!
...
async function genExec(form: Record<string, unknown>, reload?: () => void) {
  await http.post(`/pm/plan/${form.id}/generate-execution`, { created_by: auth.user?.id })
  ElMessage.success('已生成PM执行单')
  reload?.()
}

async function activate(form: Record<string, unknown>, reload?: () => void) {
  await http.post(`/pm/plan/${form.id}/activate`)
  reload?.()
}

async function loadDue() {
  const { data } = await http.get('/pm/plan/due')
