<template>
  <div class="workflow-crud">
    ...
    <AppModal v-model="itemVisible" :title="itemViewOnly ? '查看设备PM' : '设备预防性维护执行'" size="lg">
      <template v-if="currentItem">
        ...
        <el-table :data="itemResults" border size="small">
          <el-table-column prop="item_name" label="PM项目" min-width="140" />
          <el-table-column prop="item_content" label="PM内容" min-width="160" show-overflow-tooltip />
...
const modalTitle = computed(() => {
  if (openMode.value === 'execute') return '预防性维护执行'
  if (openMode.value === 'view') return '预防性维护执行详情'
  return '预防性维护执行编辑'
})
...
async function saveHeader() {
  ...
  await http.put(`/pm/execution/${exec.value.id}`, {
    ...CLIENT,
...
async function confirmItem(row: Record<string, unknown>) {
  ...
  await http.post(`/pm/execution/item/${row.id}/confirm`, CLIENT)
...
async function deleteItem(row: Record<string, unknown>) {
  ...
  await http.delete(`/pm/execution/item/${row.id}`, { params: CLIENT })
...
async function removeRow(row: Record<string, unknown>) {
  try {
    ...
    await http.delete(`/pm/execution/${row.id}`, { params: CLIENT })
    ElMessage.success('已删除')
    crudRef.value?.load()
  } catch (e: unknown) {
    if (e !== 'cancel' && e !== 'close') {
...
async function auditRow(row: Record<string, unknown>) {
  await http.post(`/pm/execution/${row.id}/audit`, { ...CLIENT, action: 'approve' })
