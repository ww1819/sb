<template>
  <div class="workflow-crud">
    ...
    <AppModal v-model="itemVisible" title="设备保养执行" size="lg">
...
async function startExec() {
  ...
  await http.post(`/maintain/execution/${exec.value.id}/start`, {})
...
async function submitExec() {
  if (!exec.value?.id) return
  await http.post(`/maintain/execution/${exec.value.id}/submit`, {})
...
async function withdrawExec() {
  if (!exec.value?.id) return
  await http.post(`/maintain/execution/${exec.value.id}/withdraw`, {})
...
async function auditExec() {
  if (!exec.value?.id) return
  await http.post(`/maintain/execution/${exec.value.id}/audit`, { action: 'approve' })
