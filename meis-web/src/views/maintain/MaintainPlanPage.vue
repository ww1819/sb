...
async function approve(form: Record<string, unknown>, reload?: () => void) {
  await http.post(`/maintain/plan/${form.id}/approve`, { action: 'approve', approved_by: auth.user?.userId })
  ElMessage.success('审核通过')
  reload?.()
}

async function reject(form: Record<string, unknown>, reload?: () => void) {
  await http.post(`/maintain/plan/${form.id}/approve`, { action: 'reject', approved_by: auth.user?.userId })
...
async function genExec(form: Record<string, unknown>, reload?: () => void, planItemIds?: string[]) {
  ...
  const { data } = await http.post(`/maintain/plan/${form.id}/generate-execution`, body)
  const no = data.data?.execution_no
  ElMessage.success(no ? `已生成执行单 ${no}` : '已生成保养执行单')
  reload?.()
}
