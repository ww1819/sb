<template>
  <MasterDetailPage :config="config" save-url="/asset/goods-return">
...
import { printGoodsReturnDoc } from '@/utils/printDoc'
...
async function printReturn(row: Record<string, unknown>) {
  if (!row?.id) return
  try {
    const { data } = await http.get(`/asset/goods-return/${row.id}`)
    if (data?.code != null && data.code !== 0) {
      ElMessage.error(data.message || '加载退货单失败')
      return
    }
    ...
    if (!doc.supplier_name && row.supplier_name) doc.supplier_name = row.supplier_name
    let hospital = '医疗机构'
    try {
    ...
    const ok = printGoodsReturnDoc(doc, hospital)
...
async function reviewSelected(
  ...
  if (!pending.length) {
    ElMessage.warning('所选退货单均已审核')
    return
  }
  const noWarehouse = pending.filter((r) => r.warehouse_id == null || String(r.warehouse_id).trim() === '')
  if (noWarehouse.length) {
    ElMessage.warning('请先为退货单选择仓库后再审核')
    return
  }
  await ElMessageBox.confirm(
    `确认审核选中的 ${pending.length} 张退货单？审核通过后将按明细回写设备台账为已退货。`,
    '审核'
  )
  let ok = 0
  for (const row of pending) {
    const { data } = await http.post(`/asset/goods-return/${row.id}/complete`, {})
    if (data?.code != null && data.code !== 0) {
