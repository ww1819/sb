<template>
  <div class="metrology-query-page">
    <SystemPageCard title="计量记录查询" :loading="loading" show-pager v-model:page="page" v-model:size="size" :total="total" @page-change="load">
      <template #filterBar>
        <PageFilterBar v-model:keyword="keyword" placeholder="设备名称 / 执行单号 / 证书编号" @search="onSearch" @reset="onReset">
          <template #filters>
            <el-input v-model="deviceCode" placeholder="设备编码" clearable class="filter-item" @change="onSearch" />
            <el-select v-model="resultStatus" placeholder="检定结果" clearable class="filter-item" @change="onSearch">
      ...
      <el-table v-loading="loading" :data="rows" stripe class="system-table" row-key="id" @row-dblclick="openDetail">
        ...
        <el-table-column prop="certificate_no" label="证书编号" min-width="120" />
    ...
    <AppModal v-model="visible" title="计量记录详情" size="lg">
      <template v-if="detail">
        <el-descriptions :column="2" border size="small">
          ...
          <el-descriptions-item label="检定机构">{{ detail.org_name }}</el-descriptions-item>
          <el-descriptions-item label="证书编号">{{ detail.certificate_no }}</el-descriptions-item>
        ...
        <el-table :data="results" border size="small" class="result-table">
          <el-table-column prop="item_name" label="检定项目" />
          <el-table-column prop="item_content" label="检定内容" show-overflow-tooltip />
...
async function load() {
  loading.value = true
  try {
    const { data } = await http.get('/metrology/query/page', {
...
function onReset() { keyword.value = ''; deviceCode.value = ''; resultStatus.value = ''; onSearch() }

async function openDetail(row: Record<string, unknown>) {
  const { data } = await http.get(`/metrology/query/${row.id}`)
