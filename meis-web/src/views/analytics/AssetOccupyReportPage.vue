  ...
  <div class="asset-occupy-report">
    ...
    <template v-if="activeTab === 'detail'">
      ...
      <div class="table-wrap">
        <div class="section-bar">资产占用统计-明细表</div>
        <el-table
    ...
    <template v-else>
      ...
      <div class="table-wrap">
        <div class="section-bar">资产占用统计-汇总表</div>
        <el-table
...
function onDetailExport() {
  ...
  downloadExcelHtml(table, `资产占用统计-明细表${formatExportDate()}`)
...
function onSummaryExport() {
  ...
  downloadExcelHtml(table, `资产占用统计-汇总表${formatExportDate()}`)
  ElMessage.success(`已导出 ${rows.length} 条`)
}
</script>
