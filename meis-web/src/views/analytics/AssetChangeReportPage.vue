  ...
  <div class="asset-change-report">
    ...
    <template v-if="activeTab === 'detail'">
            ...
            <el-row :gutter="16">
              ...
              <el-col :xs="24" :sm="12" :md="8">
                <el-form-item label="资产编码">
                  <el-input
                    v-model="detailFilters.assetCode"
                    clearable
                    placeholder="资产编码搜索"
                  />
                </el-form-item>
      ...
      <div class="table-wrap">
        <div class="section-bar">资产增减统计-明细表</div>
        <el-table
    ...
    <template v-else>
          ...
          <el-form :model="summaryFilters" class="filter-form" label-width="100px" @submit.prevent>
            <el-row :gutter="16">
              <el-col :xs="24" :sm="12" :md="8">
                <el-form-item label="价格区间">
                  <el-input
                    v-model="summaryFilters.priceRange"
                    clearable
                    placeholder="价格区间搜索"
                  />
                </el-form-item>
      ...
      <div class="table-wrap">
        <div class="section-bar">资产增减统计-汇总表</div>
        <el-table
        ...
        >
          ...
          <el-table-column prop="price_range" label="价格区间" min-width="160" sortable="custom" />
...
function buildMockDetailRows(count = 50): DetailRow[] {
  ...
  for (let i = 1; i <= count; i++) {
    const unit = 3500 + ((i * 17359) % 680000)
    const qty = 1 + ((i * 5) % 6)
...
function buildMockSummaryRows(details: DetailRow[]): SummaryRow[] {
  ...
  for (const name of PRICE_RANGES) {
    map.set(name, { qty: 0, amount: 0 })
  }
  for (const row of details) {
    const cur = map.get(row.price_range) ?? { qty: 0, amount: 0 }
    cur.qty += row._qty
    cur.amount += row._amount
    map.set(row.price_range, cur)
  }
  return PRICE_RANGES.map((name) => {
    const v = map.get(name) ?? { qty: 0, amount: 0 }
    return {
      price_range: name,
...
const filteredSummaryRows = computed(() => {
  const kw = summaryFilters.priceRange.trim()
  let rows = allSummaryRows.value
  if (kw) rows = rows.filter((r) => r.price_range.includes(kw))
  ...
  rows.sort((a, b) => {
    if (prop === 'quantity') return (a._qty - b._qty) * factor
    if (prop === 'amount') return (a._amount - b._amount) * factor
    return a.price_range.localeCompare(b.price_range, 'zh-CN') * factor
  })
  return rows
})
...
function onDetailExport() {
  ...
  const table = `<table border="1">
    <thead>
      <tr>
        <th>序号</th><th>价格区间</th><th>资产编码</th><th>资产名称</th><th>规格/型号</th>
        <th>数量</th><th>单价</th><th>金额(元)</th><th>生产厂家</th><th>供应商</th><th>序列号</th>
      </tr>
    </thead>
    <tbody>${body}${totalRow}</tbody>
  </table>`
  downloadExcelHtml(table, `资产增减统计-明细表${formatExportDate()}`)
...
function onSummaryExport() {
  ...
  const table = `<table border="1">
    <thead>
      <tr>
        <th>序号</th><th>价格区间</th><th>数量</th><th>金额(元)</th>
      </tr>
    </thead>
    <tbody>${body}${totalRow}</tbody>
  </table>`
  downloadExcelHtml(table, `资产增减统计-汇总表${formatExportDate()}`)
  ElMessage.success(`已导出 ${rows.length} 条`)
}
</script>
