...
**不要**把整个开发仓库或完整 `scripts\` 交给实施（里面有开发面板、编译、迁库工具等，容易误用）。

在**已构建**的仓库根目录执行：

```powershell
powershell -File scripts\pack-windows-field-kit.ps1 -Zip
```

产物：`release\windows-field-kit\`（及可选 `.zip`），内容仅为：

| 内容 | 说明 |
|------|------|
| `meis-*/target/*.jar` | 15 个后端 JAR |
| `www\` | 前端 `dist` |
| `scripts\` | **精简**：运维面板 + start/stop/status/logs/备份等 |
| `docs\windows-production-deploy.md` | 部署说明 |
| `README-实施.txt` | 给实施的一页纸 |

实施现场解压到 `D:\meis\app` 后，用 `scripts\ops-panel.ps1`（:5098）启停即可。  
**未打进现场包的**：`dev-panel`、`build.ps1`、移动端脚本、Flyway/对比库等开发工具。
...
### 6.2.1 实施运维面板（推荐给现场实施）

比开发面板（:5099）更安全、更简单：**只启停 JAR / 看状态 / 看日志**，无打包、无热加载。
...
| 项 | 说明 |
|----|------|
| 地址 | **仅** `http://localhost:5098`（不对外） |
| 口令 | `-Token` 或环境变量 `MEIS_OPS_TOKEN`；页面变更操作需填写 |
| 能力 | 启动核心 / 全部、停止全部、单服务启停重启、日志尾部 |
| 不做 | 打包、热加载、Clean、前端 npm |
| 长期 | 实施完成并验证后，仍应改为 **NSSM Windows 服务**（第七节） |

关闭面板窗口 **不会** 停止已启动的业务 JAR。
