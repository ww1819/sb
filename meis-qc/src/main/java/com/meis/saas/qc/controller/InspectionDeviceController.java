...
public class InspectionDeviceController {
    ...
    @GetMapping("/page")
    public Result<PageResult<Map<String, Object>>> page(PageQuery query, @RequestParam(required = false) Boolean inspectOnly) {
        StringBuilder where = new StringBuilder(" WHERE d.is_active = true ");
        List<Object> args = new ArrayList<>();
        if (Boolean.TRUE.equals(inspectOnly)) {
            where.append(" AND d.is_inspection_device = true ");
        }
        ...
        var rows = jdbc.queryForList("""
                SELECT d.id, d.device_code, d.device_name, d.specification, d.is_inspection_device,
                       dept.dept_name, p.id AS plan_id, p.plan_name,
                       COALESCE(p.next_due_date, p.plan_date) AS next_due_date,
                       p.approval_status AS plan_approval_status
                FROM medical_device d
                LEFT JOIN department dept ON dept.id = d.dept_id
                LEFT JOIN inspection_plan p ON p.device_id = d.id AND p.status = 'active'
    ...
    @PostMapping("/toggle")
    @Transactional
    @OperationLog(module = "inspect", description = "切换巡检设备标记")
    public Result<Void> toggle(@RequestBody Map<String, Object> body) {
        ...
        boolean flag = Boolean.TRUE.equals(body.get("is_inspection_device"));
        for (String deviceId : deviceIds) {
            jdbc.update("UPDATE medical_device SET is_inspection_device=?, updated_at=NOW() WHERE id=?::uuid", flag, deviceId);
        }
    ...
    @Transactional
    @OperationLog(module = "inspect", description = "批量生成巡检执行单")
    ...
    Map<String, Object> generateOneExecution(UUID planId, Map<String, Object> body) {
        ...
        String execNo = InspectionExecutionController.nextNo();
        jdbc.update("""
