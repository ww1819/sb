...
public class InspectionExecutionController {
    ...
    private void updatePlansAfterAudit(UUID execId) {
        var items = jdbc.queryForList("""
                SELECT plan_item_id, plan_id FROM inspection_execution_item
        ...
        for (Map<String, Object> row : items) {
            ...
            if (planId != null) {
                var plan = jdbc.queryForList(
                        "SELECT cycle_days FROM inspection_plan WHERE id=?::uuid"
            ...
            LocalDate next = cycleDays != null && cycleDays > 0
                    ? LocalDate.now().plusDays(cycleDays) : LocalDate.now().plusDays(7);
            jdbc.update("""
                    UPDATE inspection_plan_item SET last_done_date=CURRENT_DATE, next_due_date=?, updated_at=NOW()
                    WHERE id=?::uuid
                    """, next, planItemId);
            if (planId != null) {
                jdbc.update("""
                        UPDATE inspection_plan SET
                          ...
                          last_inspected_at = CURRENT_DATE, updated_at=NOW()
                        WHERE id=?::uuid
                        """, planId, planId);
            }
        }
