...
public class MetrologyDeviceController {
    ...
    @GetMapping("/page")
    public Result<PageResult<Map<String, Object>>> page(PageQuery query, @RequestParam(required = false) Boolean metrologyOnly) {
        StringBuilder where = new StringBuilder(" WHERE d.is_active = true ");
        List<Object> args = new ArrayList<>();
        if (Boolean.TRUE.equals(metrologyOnly)) {
            where.append(" AND d.is_metrology = true ");
        }
        ...
        var rows = jdbc.queryForList("""
                SELECT d.id, d.device_code, d.device_name, d.specification, d.is_metrology,
                       d.next_calibration_date, dept.dept_name, p.id AS plan_id, p.plan_name,
                       p.next_due_date, p.approval_status AS plan_approval_status, o.org_name
                FROM medical_device d
                LEFT JOIN department dept ON dept.id = d.dept_id
                LEFT JOIN metrology_plan p ON p.device_id = d.id AND p.status = 'active'
    ...
    @PostMapping("/toggle")
    @Transactional
    @OperationLog(module = "metrology", description = "切换计量设备标记")
    public Result<Void> toggle(@RequestBody Map<String, Object> body) {
        ...
        boolean flag = Boolean.TRUE.equals(body.get("is_metrology"));
        for (String deviceId : deviceIds) {
            jdbc.update("UPDATE medical_device SET is_metrology=?, updated_at=NOW() WHERE id=?::uuid", flag, deviceId);
        }
    ...
    @Transactional
    @OperationLog(module = "metrology", description = "批量生成计量执行单")
    ...
    Map<String, Object> generateOneExecution(UUID planId, Map<String, Object> body) {
        ...
        String execNo = MetrologyExecutionController.nextNo();
        jdbc.update("""
