...
> 主文档索引：`docs/meis-requirements.md` → AST-GAP-01。
...
| **配件更换流水** | **复用** `repair_workorder_segment_part`（按 device_id） | 本期 Sheet；非维修更换若需再建独立流水 |
...
- 「非维修配件更换」是否独立建单，待真实用户反馈（避免与 REP-F-02 库存扣减纠缠）。
