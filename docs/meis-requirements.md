...
**ANA-UI-04 定稿（2026-07-29；布局修订：明细/汇总切换）**

| 项 | 定稿 |
|----|------|
| **页面** | `/analytics/asset-usage` 资产使用率统计 |
| **切换** | 顶部 Tab：**明细表 / 汇总表**（左右切换）；各带独立查询条件与表格 |
| **明细查询** | 科室、资产名称；下一行资产规格；查询 / 重置 |
| **明细列** | 资产编号、科室、资产名称、使用率、资产规格、资产价格 |
| **汇总查询** | 科室、使用率≥；查询 / 重置 |
| **汇总列** | 科室、资产数量、平均使用率、资产总价 |
| **样式** | 对齐统计报表：`el-tabs` + 查询框 + `el-table border` |
| **数据** | **本期不接后台**；完整业务仍见 BACKLOG-ANA-04 |
...
- [x] ANA-UI-04 资产使用率统计：前端表样式（参考统计报表；六列示意，不接后台）
