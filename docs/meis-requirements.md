...
| **工具栏** | 「审核」在「导出」之前；须先勾选；弹窗填订单审核意见（默认「同意审核」），确认后对勾选行逐条落库 |
...
| **议价字段** | 会议地点、会议时间、申请科室（只读）、设备名称（只读）、参与部门、设备科意见、会议内容 |
| **默认** | 会议地点默认「设备科」；参与部门默认「设备科」+申请科室；已有议价记录打开时回填 |
| **落库** | `purchase_plan_item`：`bargain_meeting_location` / `bargain_meeting_time` / `bargain_participant_depts` / `bargain_dept_opinion` / `bargain_meeting_content` 及操作人快照 |
...
| **订单审核意见列** | 列表展示 `order_review_comment`；可再次审核覆盖 |
