...
- [x] DASH-UI-04 数据分析：新增设备旁增加多环 KPI 仪表（同心圆角进度环，仅前端示意）→ **已被 DASH-UI-10 取代（独立框移除）**
- [x] DASH-UI-05 日常办公：状态概览下方增加「交通方式」环形图（仅前端示意）→ **已被 DASH-UI-08 取代**
- [x] DASH-UI-06 日常办公：快捷入口支持用户自选（服务端持久化，仅侧栏有效菜单可选）
- [x] DASH-UI-07 日常办公：快捷入口 5 列×2 行视口，超出自动/手动左右翻页（上限 45）
...
| 项 | 定稿 |
|----|------|
| **布局** | 左侧浅色底 + 分层立体插画；右侧登录表单（租户/平台）不变 |
| **视觉** | 与 fit2cloud.com 首页 banner **同构**（6 层 PNG + float/slide 动效）；素材暂存 `meis-web/public/login-banner/` |
| **品牌** | 左侧叠加 MEIS / 医院设备管理系统文案；插画 PNG 内烘焙字样后续由产品替换微调 |
| **窄屏** | `<900px` 隐藏插画层，保留简短品牌字 + 全宽登录表单 |
| **范围外** | 不改登录鉴权逻辑；不引入 Three.js |
...
**DASH-UI-04 定稿（2026-07-21）** → **已被 DASH-UI-10 取代（独立 Multiple KPI gauge 框移除）**
...
| **Profile** | ~~`admin` 在 `newDevice` 后插入 `kpiGauge`~~ → 见 DASH-UI-10 |

**DASH-UI-05 定稿（2026-07-22）** → **已被 DASH-UI-08 取代（废止交通方式示意）**
...
| **实现** | ~~`TrafficWayChart` + `buildTrafficWayOption`~~ → 见 DASH-UI-08 |

**DASH-UI-06 定稿（2026-07-28；上限修订见 DASH-UI-07）**

| 项 | 定稿 |
|----|------|
| **位置** | 日常办公 →「快捷入口」标题旁 **设置** 按钮 |
| **候选** | 侧栏该账号 **有效菜单**（`/system/menus/effective` 叶子菜单，有 path）；无权限不可勾选 |
| **持久化** | 租户 `sys_user.preferences.quickEntryPaths`（与 RBAC `permissions` 分离）；换机跟随账号 |
| **上限** | 最多 **45** 项；超出禁止勾选并提示（原 12，DASH-UI-07 上调） |
| **展示** | 有配置按保存顺序展示；无配置回落角色 profile `quickPaths`；均与当前有效菜单 path 求交（权限收回后自动不显示） |
| **保存校验** | 后端去重保序、≤45、每个 path 须属当前用户有效菜单 |

**DASH-UI-07 定稿（2026-07-28；布局修订为 5×2）**

| 项 | 定稿 |
|----|------|
| **视口** | 快捷入口一屏 **5 列 × 2 行**（每页最多 10 项）；卡片紧凑为上图标 + 标题（无副标题） |
| **翻页** | 超过 10 项按保存顺序分页；视口高度固定为 2 行 |
| **自动播放** | 多页时约每 5 秒左右翻页循环；悬停区域暂停（对齐 DASH-UI-01） |
| **手动** | 支持鼠标拖拽 / 触控左右滑动切页；页码指示点可点选 |
| **上限** | 与 DASH-UI-06 一并上调为 **45**（最多约 5 页） |
| **实现** | `QuickEntryGrid`；设置弹窗 / 后端 `QUICK_ENTRY_MAX` 同步 45 |

**DASH-UI-08 定稿（2026-07-28；位置修订：与品牌并排，维修趋势下移）**

| 项 | 定稿 |
|----|------|
| **位置** | **数据分析** Tab：与「品牌 TOP10」**同排各半宽**（插在设备状态三列之后）；「维修趋势」与「新增设备」同排；无 origin/brand 时回落跟新增设备并排 |
| **维度** | 按责任科室（`medical_device.dept_id` → `department.dept_name`）；无科室记「未分配」 |
| **数量** | 设备台数 `COUNT(*)`（非原值）；占比 = 该项台数 / 合计台数 |
| **展示** | 空心环形饼图；外侧标签 + Tooltip 含科室/数量/占比；右侧图例；中心文案「资产分布」 |
| **截断** | 降序 TOP 10，其余合并「其他」 |
| **数据** | `/api/analytics/dashboard` 的 `deviceByDept`；读接口过滤已软删设备与科室 |
| **实现** | `AssetDistributionChart` + `buildAssetDistributionOption` |

**DASH-UI-09 定稿（2026-07-29）**

| 项 | 定稿 |
|----|------|
| **位置** | 数据分析 Tab「设备状态」环形图 |
| **口径** | 与固定资产字典 `device_status` 一致：正常 / 在用 / 维修中 / 已报废 / 已维修待验收（有数据才出扇区） |
| **展示** | 扇区与图例显示 **中文标签**（前端 `loadDict('device_status')`，附录 R） |
| **排除** | **已退货** `returned` 不计入（对齐资产登记列表隐藏已退货） |
| **软删** | `/analytics/dashboard` 的 `deviceStatus` 过滤已软删设备 |

**DASH-UI-10 定稿（2026-07-29）**

| 项 | 定稿 |
|----|------|
| **位置** | 数据分析 Tab「设备状态」卡片（原环形饼图位） |
| **展示** | 改用 **Multiple KPI gauge** 同心圆角进度环：每环对应一种设备状态占比；图例显示「状态 + 百分比」；中心「设备总数」+ 台数 |
...
- [x] AST-UI-14 资产登记编辑/查看 Tab 拆分与业务 Sheet 齐套（修订附录 P.2）
...
- [x] AST-UI-25 资产登记查看态：侧栏/内容区滚动与溢出（勿盖滚动条）
- [x] AST-EXP-01 使用到期推算（启用优先）+ 推算方式列/备注
- [x] MOB-CHANNEL-03 移动端增删改须扫描补强途径（约定；缺列必补+标签打印流水）
- [x] MOB-CHANNEL-04 创建/改/删日志须写途径（与业务表同事务）
- [x] PLT-REF-CODE-01 强业务外键列表/详情显示「编码 名称」（采购计划例外）
- [x] MOB-SYNC-01 多端已有功能：字段/业务分支扩展须 Web 与移动端同批
...
- [x] AST-OWN-01 归属区间表 + 查看态「归属历史」Sheet + 业务写回点
- [x] AST-OWN-02 编辑态「补录归属历史」Sheet（含确认生效）
- [x] AST-BF-01 台账历史补录「保存→确认→生效后不可改删」（归属/非维修换件）
- [x] AST-PART-01 编辑态「非维修配件更换」Sheet + 查看「配件更换记录」聚合（含确认生效）
- [x] AST-SRC-01 混合来源查看 Sheet「生成方式」列（配件/归属/位置/处置）
- [x] AST-LOC-01 楼层/房间位置区间 + 按钮真变更/纠错 + 「位置历史」Sheet（补录 Sheet 可选未做）
- [x] AST-DISP-01 处置记录聚合 Sheet
- [x] AST-UI-22 资产登记业务 Sheet 状态/字典列中文
- [x] AST-UI-23 编辑态「变更所属科室」按钮（真变更 / 纠错）
- [x] PLT-DEV-SHEET-01 台账单据/计划类 Sheet：表头对齐 + 下钻约定 + **混合来源「生成方式」**

**AST-OWN-01 / AST-OWN-02 / AST-LOC-01 / AST-DISP-01 / AST-UI-22 / AST-UI-23 / PLT-DEV-SHEET-01 定稿（2026-08-01）**

> **状态**：**P0–P2 主路径已实现**（2026-08-01）。迁库须重启 meis-tenant 应用 V1 新表 + R__ 字典。位置补录 Sheet、财务外部处置、Sheet 下钻路由仍可增强。

##### 缺口摘要

| 需求 | 现状 | 定稿方向 |
|------|------|----------|
| 归属（时间段） | 无专用表/Sheet | `device_ownership_period` + 查看「归属历史」 |
| 补录归属历史 | 无 | 编辑态 Sheet「补录归属历史」（AST-OWN-02） |
| 处置记录 | 分散业务单，无统一视图 | 只读聚合 API（+ 可选外部日志表预留） |
| Sheet 状态英文 | `DeviceRecordTablePanel` 直出码值 | 列接 `dictType` / 中文标签（P0） |
| 编辑改科室 | 基本信息内直接改 `dept_id` | 独立按钮 + 真变更/纠错确认（AST-UI-23） |
| 楼层/房间位置 | 台账字段可静默改；无区间史 | 按钮弹窗 + 真变更/纠错 +「位置历史」（AST-LOC-01） |
| 单据 Sheet 下钻 | 纯列表 | PLT-DEV-SHEET-01 |
| 财务处置抓取 | 仅 `kingdee_asset_code` | P3 预留，见 `BACKLOG-AST-DISP-FIN-01` |

##### 命名评估：归属历史 vs 归属变更记录

| 候选 | 评价 |
|------|------|
| 归属变更记录 | 偏「单次变更事件」，与区间模型（起止时间段）略不符 |
| **归属历史**（采用） | 覆盖业务写回、手工真变更与**补录**的整段时间线，与「补录归属历史」用语一致 |

##### AST-OWN-01 归属区间

| 项 | 定稿 |
|----|------|
| **表** | `device_ownership_period`（V1 建表 + R__ 不另建；标准七列 + W.5 姓名） |
| **字段** | `device_id`+code/name；`campus_id`+名称；`owner_type`=`warehouse`\|`dept`；`warehouse_id`/`dept_id`+名称；`effective_from`/`effective_to`（当前段 to 空）；`change_reason`；`source_biz_type/id/no`；**`confirm_status` 等**（见 AST-BF-01） |
| **规则** | 新开段前闭合上一段；**纠错**只改正当前段+台账、不新开段；**真变更**闭合+新开并写台账；业务写回/真变更段直接已确认 |
| **Sheet** | 查看态（及编辑只读区）标题「**归属历史**」；按 `effective_from` 倒序；列：院区、归属类型、仓库/科室、起止、来源单号、原因、确认状态 |
| **写回点** | 出库/退库/调拨执行或审批通过、入库完成（须先修 entry `device_id`）、AST-UI-23 真变更；报废**不**开归属段（进处置） |
| **院区** | 调拨若变更院区须同事务写 `medical_device.campus_id`（修补现缺口） |
| **与位置** | 院区+仓/科属本条；楼层/房间见 AST-LOC-01（分表，勿混进归属区间） |

##### AST-OWN-02 补录归属历史

| 项 | 定稿 |
|----|------|
| **入口** | **编辑态**独立 Sheet「**补录归属历史**」：表单录入某时间段的院区、归属类型、仓库/科室、起止时间、备注；写入同一张 `device_ownership_period` |
| **查看态** | 不提供补录表单；仅「归属历史」只读列表（可按权限后续开放补录，默认编辑态） |
| **规则** | `change_reason=import`（或 `manual_backfill`）；须填闭合区间（`effective_to` 非空）；与已有**已确认**区间不得重叠；**默认不改写**台账当前仓科/院区；覆盖「当前开放段」→ 拒绝或强制走 AST-UI-23 |
| **确认生效** | **必须**遵守 [AST-BF-01](#ast-bf-01-台账历史补录确认生效定稿2026-08-01)：先保存为待确认，确认后才计入有效历史且不可改删 |
| **权限** | 与台账编辑同权或独立权限码（实现时定） |
| **排期** | 跟 AST-OWN-01 同迭代（P1） |

##### AST-BF-01 台账历史补录确认生效（定稿 2026-08-01）

> 适用范围：资产登记**编辑态 / 方案规划**各 Sheet 的「补录」类操作评估。业务代码未开工。

**评估结论（哪些要 / 不要「保存→确认→生效后不可改删」）**

| Sheet / 操作 | 现况 | 是否采用确认生效 | 理由 |
|--------------|------|------------------|------|
| **补录归属历史**（AST-OWN-02） | 方案 | **要** | 人工补历史区间进入审计时间线，误录难纠；须草稿可改、确认后锁定 |
| **补录位置历史**（AST-LOC-01 可选） | 方案 | **要**（若做补录） | 与归属补录同构 |
| **非维修配件更换**（AST-PART-01） | 方案 | **要** | 手工补录换件进入设备履历，须确认后锁定；与维修进程换件区分 |
| 变更所属科室（AST-UI-23） | 方案 | **不要**本模式 | 弹窗已选真变更/纠错并**即时生效**；生成的区间直接为已确认 |
| 变更安装/存放位置（AST-LOC-01 按钮） | 方案 | **不要**本模式 | 同上 |
| 基本信息 / 设备档案 / 设备图片 | 已有 | **不要** | 常规主数据编辑；靠实体变更记录，非历史区间补录 |
| 电流监测 | 已有 | **不要** | 运维配置（绑标签/阈值），须可改 |
| 维保信息 | 已有 | **不要** | 主从业务维护+变更记录；非「补历史区间」；若将来做「补录维保历史段」再套用本条 |
| 查看出入库/维修/计划等业务 Sheet | 已有/规划 | **不适用** | 单据自有审批/状态机，不在台账编辑补录范围 |
| 业务写回归属/位置（出库、调拨等） | 方案 | **不适用补录流** | 系统写回直接 `confirmed`，不可改删 |

**确认生效规则（补录专用）**

| 项 | 定稿 |
|----|------|
| **状态** | `confirm_status`：`draft`（待确认）→ `confirmed`（已生效）；区间表增加 `confirmed_at` / `confirmed_by` / `confirmed_by_name` |
| **保存** | 补录新增/修改仅落 `draft`；可反复改、可删草稿 |
| **确认** | 独立「确认」动作；通过后 `confirmed`；**重叠校验仅针对已确认区间**（草稿互不影响已确认） |
| **生效后** | **禁止修改、禁止删除**（含软删入口不对业务用户开放）；纠错不得改已确认历史段——只能另补草稿再确认，或走当前值「纠错」改台账当前段 |
| **展示** | 「归属历史 / 位置历史 / 配件更换记录」等以已确认为准；编辑补录 Sheet 另列待确认草稿（状态中文） |
| **真变更即时段** | 按钮真变更写入的新段：直接 `confirmed`（弹窗即确认） |
| **生成方式** | 凡查看 Sheet 同时含业务生成与手工补录 → 遵守 [AST-SRC-01](#ast-src-01--plt-dev-sheet-01-生成方式列定稿2026-08-01) |
| **双写** | 约定包 §5.17 补录确认行 |

##### AST-PART-01 非维修配件更换（定稿 2026-08-01）

| 项 | 定稿 |
|----|------|
| **合理性** | AST-UI-21 仅展示维修进程配件；日常保养/巡检外换件、历史补录需独立流水，且勿与维修库存扣减纠缠 |
| **编辑 Sheet** | 「**非维修配件更换**」：录入配件、数量、单价/金额、供应商、更换日期、备注等（字段对齐查看列关键项，实现时定） |
| **表** | 新建 `device_part_replacement`（建议；V1+R__）：`device_id`+code/name；配件 id/code/name；数量单价金额；`supplier_id`+名称；`replaced_at`；`confirm_status` 等；`source_mode=manual_backfill`；审计七列+姓名；**不**强制扣备件库存（另议） |
| **确认生效** | **必须** AST-BF-01：保存草稿 → 确认后生效 → 不可改删 |
| **查看** | 仍用单一 Tab「**配件更换记录**」：UNION 维修 `repair_workorder_segment_part`（`source_mode=biz_doc`）+ 本表已确认行；列增「**生成方式**」 |
| **下钻** | 维修来源可进工单/进程；手工补录只读展示本行（无独立业务单） |
| **排期** | P1 可与归属同批或紧随；记 `BACKLOG-AST-PART-01` |

##### AST-SRC-01 / PLT-DEV-SHEET-01 生成方式列（定稿 2026-08-01）

| 项 | 定稿 |
|----|------|
| **触发** | 同一查看 Sheet **既有**业务单据/操作写回记录，**又有**手工补录（或其它非单据来源） |
| **要求** | 列表**靠后**增加列「**生成方式**」（中文）；码值建议：`biz_doc`=业务单据、`biz_op`=业务操作写回、`manual_backfill`=手工补录、`system`=系统生成（可按需扩展） |
| **位置** | 列顺序：业务关键列在前，「生成方式」置后（用户要求「后面添加」） |
| **适用（资产登记）** | **配件更换记录**（维修+非维修）；**归属历史**（业务写回+补录+真变更可再细分）；**位置历史**；**处置记录**（若含手工/外部） |
| **不适用** | 仅单一来源的 Sheet（如纯维修记录列表）可不加，以免空列噪音 |
| **双写** | 约定包 §5.16 |

##### AST-LOC-01 位置历史（楼层/房间）

| 项 | 定稿 |
|----|------|
| **合理性** | 与科室归属同构：静默改字段会污染审计；须区分纠错与真实移机 |
| **表** | `device_location_period`（建议）：`device_id`+code/name；`location_floor`/`room_number`（及既有安装/存放位置字段快照，实现时与台账列对齐）；`effective_from`/`effective_to`；`change_reason`；`source_biz_*`；**`confirm_status` 等**（补录时）；审计七列+姓名 |
| **当前值 UI** | 基本信息中楼层/房间等：**只读或弱化**；入口按钮「变更安装/存放位置」→ 选择器/表单 → 确定后二次确认 |
| **二次确认** | ① **生成位置历史**（真变更：闭合旧段+新开段并写台账，段直接已确认）；② **仅修正台账位置（不生成位置历史）**（纠错） |
| **Sheet** | 查看态「**位置历史**」；列：楼层、房间（及约定快照字段）、起止、来源、原因、确认状态 |
| **补录** | 可选：编辑态「补录位置历史」与 OWN-02 同构，且**必须**遵守 AST-BF-01；首版可不做 |
| **分期** | **方案本期定稿**；实现排 **P2**（`BACKLOG-AST-LOC-01`）；不阻塞 P1 归属 |
| **双写** | 真变更/纠错 + 补录确认 → 约定包 §5.17 |

##### AST-DISP-01 处置记录

| 项 | 定稿 |
|----|------|
| **原则** | 展示导致权属/在库/处置终态变化的操作；不替代原单据页 |
| **本期纳入** | 入库、出库、退库、调拨/转仓、退货、报废、盘亏/盘实不符（展示）、手工归属真变更 |
| **本期不纳入** | 公用借调（已有借调 Sheet） |
| **实现** | 首版只读 UNION 聚合 API + 统一 DTO；外部/财务写入预留 `device_disposition_log` 或 `external_asset_disposition`（P3） |
| **依赖** | 入库 complete 可靠写 `device_entry_item.device_id`（`BACKLOG-AST-ENTRY-DID-01`，P1/P2 必修） |

##### AST-UI-22 Sheet 状态中文

| 项 | 定稿 |
|----|------|
| **范围** | 资产登记查看态 `DeviceRecordTablePanel` 各业务 Sheet |
| **做法** | 列定义增加 `dictType`（或后端 `*_label`）；与 `sys_dict` / PLT-STATUS-CN-01 对齐 |
| **覆盖** | 维修 `wo_status`；计划 `approval_status`/`plan_status`；执行状态；计量 `overall_result`；借调/收费；盘点；不良事件等 |
| **分期** | **可 P0 单独上线**，不依赖归属表 |
...
| 项 | 定稿 |
|----|------|
| **UI** | 编辑态基本信息区按钮「变更所属科室」→ 科室选择 → 二次确认 |
| **选项** | ① 生成归属历史段（真变更）；② 仅修正台账科室（纠错，不新开段） |
| **表单** | `dept_id` 弱化或只读，避免静默改归属 |
| **依赖** | AST-OWN-01 表与服务 |

##### PLT-DEV-SHEET-01 单据/计划类 Sheet

| 项 | 定稿 |
|----|------|
| **列表列** | 对齐对应业务列表页表头关键列（单号、日期、状态中文、责任人等） |
| **下钻** | 行可打开同模块详情（路由或抽屉）；主从须见明细；多层见最底层；**AST-SHEET-DRILL-01**：点单号/「明细」→ 只读抽屉（头+明细）；保养/巡检/PM/计量等执行单须同时展示 `*_execution_result` |
| **无详情页** | 至少只读抽屉展示头+明细（执行类单据含结果行） |
| **生成方式** | 混合来源时须加「生成方式」列 → [AST-SRC-01](#ast-src-01--plt-dev-sheet-01-生成方式列定稿2026-08-01) |
| **范围** | 资产登记查看态单据/计划/历史聚合 Sheet；新 Sheet 默认遵守 |
| **双写** | 约定包 §5.16 |
...
| 期次 | 内容 |
|------|------|
| **P0** | AST-UI-22；PLT-DEV-SHEET-01/AST-SRC-01 文档（已完成）+ 下钻/生成方式改造可同批或紧随 |
| **P1** | AST-OWN-01/02+BF-01；AST-UI-23；**AST-PART-01**；调拨写 campus；entry `device_id`；混合 Sheet 生成方式列 |
| **P2** | AST-DISP-01；盘亏展示；**AST-LOC-01** |
| **P3** | 财务外部处置；盘亏是否写回台账；位置补录 Sheet（若做） |
...
| **字段区** | 品牌/规格/型号/序列号/注册证号/生产·验收·启用日期/设备状态；科室/仓库/安装位置/存放位置/责任人；厂家与供应商编码名称；原值/净值/购置日期 |
...
**AST-UI-20 定稿（2026-08-01）**

| 项 | 定稿 |
|----|------|
| **范围** | AST-UI-19 资产卡片工具栏增强 |
| **方向** | 打印/预览可选 **纵向 / 横向**（`@page` A4 portrait/landscape；屏幕网格随方向略调） |
| **导出** | **PNG**、**PDF**（`html2canvas` + `jspdf`）；文件名 `资产卡片_{device_code}.*` |
| **与打印记录** | 导出不强制写流水；点「打印卡片」仍写 `asset_card` 流水 |

**AST-UI-21 定稿（2026-08-01；扩展见 AST-PART-01）**

| 项 | 定稿 |
|----|------|
| **范围** | 资产登记 **查看态** Tab「配件更换记录」（在「维修记录」后） |
| **数据（已落地）** | 读 `repair_workorder_segment_part`（`device_id`）联查配件档案/供应商/工单/进程类型 |
| **API** | `GET /api/asset/device/{id}/spare-replacements`（实现 AST-PART-01 时扩展为聚合 + `source_mode`） |
| **扩展（方案）** | 并入非维修已确认行；列增「生成方式」；编辑态补录见 **AST-PART-01** |

**AST-GAP-01 定稿（2026-08-01）**

| 项 | 定稿 |
|----|------|
| **产出** | 缺口分析全文见 [`docs/meis-requirements-asset-ledger-gap.md`](meis-requirements-asset-ledger-gap.md) |
| **主表建议补** | UDI、资产管理员/临床责任人（UUID+姓名）、器械类别、关键等级、折旧方法、购置方式等（P0–P2 见专文） |
| **宜从表** | 证照附件、培训授权、位置变更史、非维修换件流水；特种/维保/计量执行等已有对象勿堆主表 |
| **本期** | **只分析不批量加列**；实施排 `BACKLOG-AST-GAP-01` |

**AST-GAP-REVIEW-01 定稿（2026-08-01）— 三甲再审（双部分 + 流程）**

| 项 | 定稿 |
|----|------|
| **产出** | [`docs/meis-requirements-tertiary-gap-review.md`](meis-requirements-tertiary-gap-review.md) |
...
| 项 | 定稿 |
|----|------|
| **含义** | 「前三页」= 列表默认 `pageSize=20` 的前 3 页展示，约 **60** 台；并填满查看态可落库 Sheet |
| **脚本** | [`data/seed/demo_asset_ledger.sql`](../data/seed/demo_asset_ledger.sql)（**手工**对租户 schema 执行，不进 Flyway） |
| **执行策略** | **先物理删除**种子命名空间内数据 → **再全量插入**；改脚本后可直接重跑 |
| **命名空间** | 设备 `SD2408****`（兼容清理旧 `DEMO-AST-*`）；配件 `SDSP****`；业务单号以 `SD…` 开头（兼容旧 `DEMO-*`） |
| **文案** | 界面可见名称/备注/故障描述等**不出现**「演示」「测试」等字样，贴近院端真实业务表述 |
| **样板机** | **`SD24080001`**（多参数监护仪）挂齐可落库 Sheet |
| **已覆盖 Sheet** | 基本信息 / 资产卡片；维保信息；资产标签；维修记录；配件更换记录；保养/巡检/计量/PM 的计划与记录；借调记录与费用；盘点记录；不良事件；电流读数（默认今日）；电流标签绑定记录 |
| **不可 SQL 填充** | **设备档案**、**设备图片**：当前前端内存态、无落库表（见衍生） |
| **注意** | 执行前 `SET search_path TO <tenant_schema>`；删除范围仅限上述命名空间，勿改前缀以免误伤业务数据 |
| **衍生（待确认）** | 若演示必须展示档案/图片：需先做附件落库，再扩种子；可记 `BACKLOG-AST-ARCHIVE-01` |
...
| **代码落点** | 前端 `meis-web/src/config/deviceLedgerFields.ts`、`utils/deviceLedgerSnapshot.ts`、`components/table/DeviceLedgerTableColumns.vue`；后端 `DeviceLedgerSelectSupport`；约定包 §5.15 |
| **推广** | 新功能默认按本约定；入库新建台账明细（`device_entry_item`）不以台账选择为主，不强制全列；后续增删列/筛选须同步改下表 + 上述共用组件 |

##### 公共字段清单（PLT-DEV-LIST-01）

| 顺序 | 字段 prop | 中文 | 说明 |
|------|-----------|------|------|
| 1 | `device_code` | 资产编码 | 选择器/明细必显 |
| 2 | `device_name` | 资产名称 | 选择器/明细必显 |
| 3 | `brand` | 品牌 | |
| 4 | `specification` | 规格 | 列在 `DeviceLedgerTableColumns`；明细 schema 可另有业务规格列 |
| 5 | `model` | 型号 | |
| 6 | `registration_no` | 医疗器械注册证号 | |
| 7 | `production_date` | 生产日期 | |
| 8 | `acceptance_date` | 验收日期 | |
| 9 | `enable_date` | 启用日期 | |
| 10 | `serial_number` | 序列号 | |
| 11 | `has_power_tag` | 是否有电流标签 | 联查电流标签存在性 |
| 12 | `power_tag_code` | 电流监测编码 | 联查 `power_tag.tag_code` |
| 13 | `dept_name` | 科室 | |
| 14 | `warehouse_name` | 仓库 | |
| 15 | `install_location` | 安装位置 | 读侧拼装：楼宇名 + 楼层 + 房间号 |
| 16 | `location_detail` | 存放位置 | 台账 `location_detail` |
| 17 | `responsible_person_name` | 责任人 | 映射台账 `use_dept_head`（领用科室负责人） |
| 18 | `manufacturer_code` | 生产厂家编码 | |
| 19 | `manufacturer_name` | 生产厂家名称 | |
| 20 | `supplier_code` | 供应商编码 | |
| 21 | `supplier_name` | 供应商名称 | |
| 22 | `device_status` | 设备状态 | 字典 `device_status` |

> 明细表已有编码/名称时，`deviceLedgerDetailFields` 从品牌起插在编码名称之后（不含 1–2）。场景列（单价、到期日等）跟在本清单之后。

##### 公共筛选清单（PLT-DEV-LIST-01）

| 级别 | 筛选项 | 参数习惯 / 说明 |
|------|--------|-----------------|
| **必选** | 资产编码 | `device_code` / `deviceCode`；模糊 |
| **必选** | 名称 | `device_name` / `deviceName`；**文字 + 拼音简码**模糊 |
| **必选** | 规格 | `specification` |
| **必选** | 型号 | `model` |
| **必选** | 序列号 | `serial_number` / `serialNumber` |
| **必选** | 电流标签编码 | `power_tag_code` / `powerTagCode` |
| **必选** | 所属科室 | `dept_name` / `deptName` 或业务已定科室 `deptId` |
| **建议** | 设备状态 | `device_status` / `deviceStatus` |
| **建议** | 仓库 | 按页面侧重点 |
| **建议** | 生产厂家 / 供应商 | 按页面侧重点 |
...
| 项 | 定稿 |
|----|------|
| **范围** | 资产登记 `/asset/device` |
| **列表** | 验收日期起止筛选（`acceptance_date`）；列表展示验收日期 |
| **编辑 Tab** | 「设备图片」后追加 **一个** Tab「电流监测」（修订 AST-UI-14：编辑态 3→4 Tab） |
| **绑定** | 仅选**已有且未绑定设备**的标签（编码搜索）；支持解绑；新建标签去「电流标签」菜单（不页内新建） |
| **一设备一标签** | 复用 PWR-B-03；已被占用则拒绝并提示 |
| **待机上下限** | 本 Tab 可直接改 `standby_current_max_ma/min_ma`；已绑标签可立即回写；未绑随抽屉保存 |
| **监测记录** | 同 Tab 下半区复用设备电流读数面板 |
| **新增态** | 无 `deviceId` 时提示「请先保存设备」 |
| **查看态** | 仍保留「电流读数」「电流标签绑定记录」，不改顺序 |

**AST-UI-16 定稿（2026-07-31）**

| 项 | 定稿 |
|----|------|
| **范围** | 资产登记列表 `/asset/device`（库存查询等同接口页不强制同步本期） |
| **原「起/止」** | 对应字段 **`enable_date`（启用日期）**；控件文案改为「启用日期起」「启用日期止」 |
| **新增筛选** | ① 是否有电流监测标签（是/否）② 电流监测标签编码（模糊）③ 录入时间起止 ④ 生产日期起止 |
| **有标签口径** | 存在未软删且 `device_id` 指向该设备的 `power_tag` 记为「是」；否则「否」 |
| **标签编码** | 取该设备一条有效绑定的 `tag_code`（多条时取最近更新一条）；筛选对 `tag_code` ILIKE |
| **录入时间** | `medical_device.created_at`（按日期闭区间：起日 00:00 ～ 止日当日含） |
| **生产日期** | `medical_device.production_date` |
| **列表追加列** | 是否有电流监测标签、电流监测标签编码、录入时间 |
| **批量/全选** | `/ids` 与列表共用同一套筛选条件 |

**AST-UI-15 定稿（2026-07-28）**

| 项 | 定稿 |
|----|------|
| **范围** | 资产登记列表 `/asset/device` |
| **全选** | 工具栏「全选」= 勾选**当前查询全部结果**（拉 id，上限 5000；跨页保留）；**不**恢复「已选 N / 全选当页」提示条 |
| **取消全选** | 工具栏清空跨页勾选 |
| **批量修改** | `promptListActionScope`：仅勾选行 / 全部查询结果；白名单字段勾选才更新 |
| **可改字段** | 科室、管理科室、设备状态、风险等级、院区、楼宇、仓库；布尔：生命支持/应急/计量/共享/保养/巡检/PM |
| **禁止** | 资产编码、名称、金额等主数据敏感项不在批量范围 |
| **追加列** | 注册证号、生产厂家编码/名称、供应商编码/名称、生产日期、使用日期是否已到、使用到期日期、使用到期剩余天数 |
| **到期口径** | `service_expiry_date`：是否已到 = 到期日≤今天；剩余天数 = `max(0, 到期日−今天)`，已过期为 0；到期日空则两列空 |
| **列展示** | 新列默认全部显示（可横滚） |

**AST-UI-14 定稿（2026-07-28）**
...
| **查看 Tab 顺序** | 基本信息 → 资产卡片 → 设备档案 → 设备图片 → **维保信息** → 资产标签 → 维修记录 → **配件更换记录** → 保养记录 → 保养计划 → 巡检记录 → 巡检计划 → 计量记录 → 计量计划 → PM维护记录 → PM维护计划 → 借调记录 → 借调费用 → 盘点记录 → 不良事件 → 电流读数 → 电流标签绑定记录；（规划）**归属历史**（AST-OWN-01）→ **位置历史**（AST-LOC-01）→ **处置记录**（AST-DISP-01）建议插在盘点与不良事件之间或借调之后，实现时定序 |
| **编辑 Tab（规划）** | 「**补录归属历史**」（AST-OWN-02）；「**非维修配件更换**」（AST-PART-01）；位置真变更走基本信息按钮（AST-LOC-01） |
| **数据** | 各业务 Sheet 按设备查询并展示；空壳 Tab 须接通 `loadUrl`/设备侧 API；单据/计划类遵守 **PLT-DEV-SHEET-01** |
| **命名** | 保持「设备图片」；「电流度数」改为「电流读数」 |
| **状态展示** | 业务 Sheet 状态/字典列须中文（AST-UI-22），禁止裸英文码 |
...
- [x] REP-STAT-01 筛「已验收」含验收后结案（closed+verified）；列表展示对齐
...
| └ 资产使用率统计 | `/analytics/asset-usage` | ANA-UI-04 前端表样式；业务见 BACKLOG-ANA-04 |
| └ 价值结构分析表 | `/analytics/value-structure` | ANA-UI-05 前端表样式；业务见 BACKLOG-ANA-05 |
| └ 折旧到期汇总 | `/analytics/depr-due` | ANA-UI-06 前端表样式；业务见 BACKLOG-ANA-06 |
| └ 资产折旧统计 | `/analytics/depr-stats` | ANA-UI-07 前端表样式；业务见 BACKLOG-ANA-07 |
| └ 折旧明细比例 | `/analytics/depr-ratio` | ANA-UI-08 前端表样式；业务见 BACKLOG-ANA-08 |
| └ 折旧详情 | `/analytics/depr-detail` | 占位；业务见 BACKLOG-ANA-09 |
| └ 资产增减统计 | `/analytics/asset-change` | ANA-UI-09 前端表样式；业务见 BACKLOG-ANA-10 |
| └ 资产在用统计 | `/analytics/asset-occupy` | ANA-UI-10 前端表样式；业务见 BACKLOG-ANA-11 |
| └ 资产移动统计表 | `/analytics/asset-transfer` | ANA-UI-11 前端表样式；业务见 BACKLOG-ANA-12 |
...
| **新增叶子（9）** | 资产使用率统计、价值结构分析表、折旧到期汇总、资产折旧统计、折旧明细比例、折旧详情、资产增减统计、资产在用统计（原「资产占用统计」）、资产移动统计表（原「资产异动统计」） |
| **本期范围** | 仅菜单入口 + 前端占位页；业务报表另开 BACKLOG-ANA-04~12 |
| **授权** | 写入 `sys_package_menu`（standard/flagship）与 `sys_tenant_menu` |

**ANA-UI-04 定稿（2026-07-29；2026-07-30 布局对齐 ANA-UI-05）**

| 项 | 定稿 |
|----|------|
| **页面** | `/analytics/asset-usage` 资产使用率统计 |
| **切换** | 顶部 Tab：**明细表 / 汇总表**；与 ANA-UI-05 **同一套结构与样式**（位置/宽高/间距/按钮） |
| **明细查询** | 科室、资产名称；下一行单独「资产规格」；查询 / 重置；「查询条件」标题条 `#d6ebf8` |
| **明细列** | 序号、资产编号、科室、资产名称、使用率、资产规格、资产价格；编号/科室/名称/使用率支持升序降序 |
| **明细底栏** | 合计：总数量、总金额、当前页面数量、当前页面金额；底栏 `#d6ebf8`；分页 **10 / 20 / 30 / 50** |
| **汇总查询** | 科室、使用率≥；查询 / 重置 |
| **汇总列** | 序号、科室、资产数量、平均使用率、资产总价；均可排序；同款底栏与分页 |
| **样式** | 对齐 ANA-UI-05：浅蓝表头、隔行白/`#fef5e7`、悬停 `#e6f7ff`、表格线 `#d0d7de`；明细框撑满剩余高度；底留白约 16px；明细/汇总查询区重置后 **导出**（文件名含当天 `YYYYMMDD`，含合计行） |
| **数据** | **本期不接后台**（前端示意数据）；完整业务仍见 BACKLOG-ANA-04 |
| **实现** | `AssetUsageReportPage.vue` |

**ANA-UI-05 定稿（2026-07-29）**

| 项 | 定稿 |
|----|------|
| **页面** | `/analytics/value-structure` 价值结构分析表 |
| **布局** | 对齐 ANA-UI-04：顶部 **明细表 / 汇总表** Tab；各带查询条件框 + `el-table border`；紧凑间距 |
| **明细查询** | 设备分类编号、设备类型；下一行单独「数量」查询；查询 / 重置 |
| **明细列** | 序号、设备分类编号、设备类型、数量、期末原值（元）、期末累计折旧(元)、期末净值（元)、原值百分比(%)、净值率(%)；其中分类编号/类型/数量/期末原值支持升序降序 |
| **明细底栏** | 合计行：总数量、总金额、当前页面数量、当前页面金额；底栏背景与「查询条件」表头一致（品牌色报表 token `--meis-report-header-bg`，默认浅蓝）；分页可选每页 **10 / 20 / 30 / 50** 条；明细/汇总查询区重置后 **导出**（`价值结构分析表-明细表/汇总表YYYYMMDD.xls`，含加粗合计行；表头色取当前品牌色） |
| **表上说明** | 明细/汇总表上方标题行（无背景）；「注：…」为**红色**；「查询条件」标题条 / 表头 / 悬停 / 隔行均走 `--meis-report-*`（偏好「系统颜色」驱动，与菜单主色统一） |
| **汇总查询** | 设备类型、净值率≥；查询 / 重置 |
| **汇总列** | 设备类型、数量合计、期末原值合计（元）、期末净值合计（元）、原值百分比(%)、平均净值率(%) |
| **数据** | **本期不接后台**；完整业务仍见 BACKLOG-ANA-05 |
| **实现** | `ValueStructureReportPage.vue` |

**PLT-UI-SURFACE-01 全站表面（2026-07-31）**

| 项 | 定稿 |
|----|------|
| **目标** | 全站列表页查询区/表格表面与弹窗，统一为 ANA-UI-05（价值结构分析表）同款风格 |
| **样式** | `meis-web/src/styles/list-surface.css`（`--meis-report-*`；外框圆角 2px） |
| **列表壳** | `SystemPageCard` **默认** `variant="report"`（查询色条「查询条件」+ 表题色条 + `meis-table-wrap`）；`filterBar` 纳入 query-box |
| **筛选条** | `PageFilterBar` 去掉灰底卡片，由外层 query-box 承托 |
| **弹窗** | `AppModal` / `FormDrawer` **默认** `variant="report"`（色条标题、零内边距贴边抽屉、报表底栏） |
| **覆盖范围** | 经 `CrudPage` / `SystemPageCard` / `AppModal` 的菜单页自动生效；已手写 `query-box` 的分析报表保持原样 |
| **例外** | 大屏/工作台等非列表壳页面不强制；若单页需旧样式可显式 `variant="default"` |
| **验收** | 任意字典/业务列表：查询色条+表头米黄隔行+底部分页；新增/编辑抽屉与列表同色系、无四周缝隙 |

**ANA-UI-06 定稿（2026-07-30）**

| 项 | 定稿 |
|----|------|
| **页面** | `/analytics/depr-due` 折旧到期汇总 |
| **切换** | 顶部 Tab：**资产折旧汇总(资金)** / **折旧汇总(分级)** / **折旧汇总(状态)**；与 ANA-UI-05 同套结构与样式 |
| **资金 Tab** | 查询：资金来源；二级表头：序号/资金来源跨行；**期初 / 本期增加 / 本期减少 / 期末** 各组下「资产金额(元)、累计折旧(元)」；标题旁 **?** 悬停显示红色「数据来源公式解析」；重置后 **导出**（Excel，文件名 `资产折旧汇总(资金)YYYYMMDD`，取当天）；底栏合计 + 分页 |
| **分级 Tab** | 查询：资产分级；列：序号、原值(元)、本期折旧(元)、累计折旧(元)、净值(元)、计提原值(元)；同款底栏分页 |
| **状态 Tab** | 查询：使用状态；列：序号、科室编码、科室名称、使用状态、原值(元)、累计折旧(元)、净值(元)；同款底栏分页 |
| **样式** | 对齐 ANA-UI-05：浅蓝表头、隔行白/`#fef5e7`、悬停 `#e6f7ff`、表格线 `#d0d7de`；明细框撑满剩余高度 |
| **数据** | **本期不接后台**（前端示意数据）；完整业务仍见 BACKLOG-ANA-06 |
| **实现** | `DeprDueReportPage.vue` |

**ANA-UI-07 定稿（2026-07-30）**

| 项 | 定稿 |
|----|------|
| **页面** | `/analytics/depr-stats` 资产折旧统计 |
| **切换** | 顶部 Tab：**明细表 / 汇总表**；与 ANA-UI-05 **同一套结构与样式** |
| **明细/汇总查询** | 价值区间；查询 / 重置 / **导出**（`资产折旧统计-明细表/汇总表YYYYMMDD.xls`，含加粗合计行） |
| **列** | 序号、资产价值区间、原值(元)、本期折旧(元)、累计折旧(元)、净值(元) |
| **区间取值** | 万元以下、1-10万、10-20万、20-50万、50万元以上 |
| **样式** | 对齐 ANA-UI-05：查询条 `#d6ebf8`、表头浅蓝、隔行/悬停、底栏淡蓝合计、分页 10/20/30/50 |
| **数据** | **本期不接后台**（前端示意）；完整业务仍见 BACKLOG-ANA-07 |
| **实现** | `DeprStatsReportPage.vue` |

**ANA-UI-08 定稿（2026-07-30）**

| 项 | 定稿 |
|----|------|
| **页面** | `/analytics/depr-ratio` 折旧明细比例 |
| **切换** | 顶部 Tab：**明细表 / 汇总表**；与 ANA-UI-05 **同一套结构与样式** |
| **明细查询** | 设备编号、资产名称；下一行使用科室；查询 / 重置 / 导出 |
| **明细列** | 序号、设备编号、资产名称、规格/型号、单位、使用科室、月折旧率(%)、月折旧金额（元）、原值（元）、累计折旧金额（元）、净值（元）、购入日期、折旧科室、折旧比例、已折旧月份、存放地点；列用 `min-width` 保证完整显示，宽屏时自动撑满明细框无右侧大块留白 |
| **汇总查询** | 使用科室；查询 / 重置 / 导出 |
| **汇总列** | 序号、使用科室、资产数量、原值合计、月折旧合计、累计折旧合计、净值合计 |
| **导出** | `折旧明细比例-明细表/汇总表YYYYMMDD.xls`，含加粗合计行 |
| **横向滚动** | 明细表用 `height=100%` 由表格自管滚动，横向滚动条固定在合计栏正上方；列用 `min-width` 宽屏撑满、窄屏可横滑；滚动条高度约 8px |
| **数据** | **本期不接后台**；完整业务仍见 BACKLOG-ANA-08 |
| **实现** | `DeprRatioReportPage.vue` |

**ANA-UI-09 定稿（2026-07-30）**

| 项 | 定稿 |
|----|------|
| **页面** | `/analytics/asset-change` 资产增减统计 |
| **切换** | 顶部 Tab：**明细表 / 汇总表**；与 ANA-UI-05 **同一套结构与样式**（Tab 位置/宽高、查询框与顶边距、明细框撑满、按钮大小位置） |
| **明细查询** | 价格区间、资产编码；下一行资产名称；查询 / 重置 / 导出 |
| **明细列** | 序号、价格区间、资产编码、资产名称、规格/型号、数量、单价、金额(元)、生产厂家、供应商、序列号 |
| **汇总查询** | 价格区间；查询 / 重置 / 导出 |
| **汇总列** | 序号、价格区间、数量、金额(元) |
| **导出** | `资产增减统计-明细表/汇总表YYYYMMDD.xls`，含加粗合计行 |
| **样式** | 对齐 ANA-UI-05：查询条 `#d6ebf8`、表头浅蓝、隔行白/`#fef5e7`、悬停 `#e6f7ff`、表格线 `#d0d7de`；底栏淡蓝合计 + 分页 10/20/30/50 |
| **数据** | **本期不接后台**（前端示意）；完整业务仍见 BACKLOG-ANA-10 |
| **实现** | `AssetChangeReportPage.vue` |

**ANA-UI-10 定稿（2026-07-30；菜单更名 2026-07-30）**

| 项 | 定稿 |
|----|------|
| **页面** | `/analytics/asset-occupy` **资产在用统计**（原「资产占用统计」；路由/`menu_code` 不变） |
| **切换** | 顶部 Tab：**明细表 / 汇总表**；与 ANA-UI-05 **同一套结构与样式**（Tab 位置/宽高、查询框与顶边距、明细框撑满、按钮大小位置） |
| **明细查询** | 价格区间、科室；下一行资产名称；查询 / 重置 / 导出 |
| **明细列** | 序号、价格区间、科室、资产编码、资产名称、规格/型号、数量、单价、金额(元)、生产厂家、供应商、序列号 |
| **汇总查询** | 科室；查询 / 重置 / 导出 |
| **汇总列** | 序号、科室、数量、金额(元) |
| **导出** | `资产在用统计-明细表/汇总表YYYYMMDD.xls`，含加粗合计行 |
| **样式** | 对齐 ANA-UI-05：查询条 `#d6ebf8`、表头浅蓝、隔行白/`#fef5e7`、悬停 `#e6f7ff`、表格线 `#d0d7de`；底栏淡蓝合计 + 分页 10/20/30/50 |
| **数据** | **本期不接后台**（前端示意）；完整业务仍见 BACKLOG-ANA-11 |
| **实现** | `AssetOccupyReportPage.vue`；菜单名见 `R__menus.sql` |

**ANA-UI-11 定稿（2026-07-30）**

| 项 | 定稿 |
|----|------|
| **页面** | `/analytics/asset-transfer` **资产移动统计表**（原「资产异动统计」；路由/`menu_code` 不变） |
| **切换** | 顶部 Tab：**明细表 / 汇总表**；与 ANA-UI-05 **同一套结构与样式**（Tab 位置/宽高、查询框与顶边距、明细框撑满、按钮大小位置） |
| **明细查询** | 转科单号、资产名称；下一行调出科室；查询 / 重置 / 导出 |
| **明细列** | 序号、转科单号、申请日期、转科日期、资产编号、资产序列号、资产名称、条码号、资产品牌、型号、单位、原值（元）、残值（元）、调出科室、原负责人、调入科室、现负责人、转科原因、转科文号、年限、折旧状态 |
| **汇总查询** | 科室；查询 / 重置 / 导出 |
| **汇总列** | 序号、科室、数量、金额(元) |
| **导出** | `资产移动统计表-明细表/汇总表YYYYMMDD.xls`，含加粗合计行 |
| **样式** | 对齐 ANA-UI-05：查询条 `#d6ebf8`、表头浅蓝、隔行白/`#fef5e7`、悬停 `#e6f7ff`、表格线 `#d0d7de`；底栏淡蓝合计 + 分页 10/20/30/50 |
| **数据** | **本期不接后台**（前端示意）；完整业务仍见 BACKLOG-ANA-12 |
| **实现** | `AssetTransferReportPage.vue`；菜单名见 `R__menus.sql` |

**ANA-UI-12 定稿（2026-07-30；页面布局 2026-07-30）**
...
| **布局** | **上**：查询条件（设备类别、设备名称、价格区间起止、查询、**费用 Excel 导入**）；**下左**：固定资产列表（加宽约半屏：编号/名称/型号/序列号/类别/价格区间/原值，点选回填右侧）；**下右**：费用维护表单（统计年月、记录人/时间、类别与价格区间、名称；人次/分摊勾选；收入与各项费用四列录入；保存/清空；公式说明） |
| **Excel 导入弹窗** | 查询旁入口；弹窗内：**下载模板**（前端生成 .xls）、上传后 **解析模板**（预览行）、**导入**（本期示意成功，不落库）；完整落库见 BACKLOG-ANA-13 |
| **数据** | **本期不接后台**（前端示意）；完整读写与分摊规则见 BACKLOG-ANA-13 |
...
- [x] ANA-UI-04 资产使用率统计：前端表样式对齐 ANA-UI-05（明细/汇总同套布局；示意数据，不接后台）
- [x] ANA-UI-05 价值结构分析表：同布局（明细/汇总切换）；明细八列示意，不接后台
- [x] ANA-UI-06 折旧到期汇总：三 Tab（资金/分级/状态）前端样式对齐 ANA-UI-05；示意数据，不接后台
- [x] ANA-UI-07 资产折旧统计：明细/汇总同套布局；价值区间五行示意；导出；不接后台
- [x] ANA-UI-08 折旧明细比例：明细/汇总同套布局；十六列明细示意；导出；不接后台
...
- [x] ANA-UI-12 费用手工登记：菜单 + 上搜下左资产/右维护布局 + 费用 Excel 导入弹窗（示意，不接后台）
...
| PWR-B-03 | **一设备一标签** | 同一 `device_id` 同时仅允许一个未软删标签绑定（解绑后可再绑其他标签） | P0 |

**PWR-B-03 一设备一标签（Q-03 ✓，2026-07-29）**

- 保存/改绑时：若目标设备已被**其他未软删**标签占用，拒绝并提示该设备已绑定标签编码。
- 解绑（`device_id` 置空）允许；软删标签不占用设备。
- Web 标签维护与移动端 MOB-PWR-01 **同一校验**。
...
- [x] MOB-F-08 公用设备借调（申请/审核/归还/归还审核+借出；见 [MOB-SHR-01](#mob-shr-01-定稿2026-07-28)）
- [x] MOB-F-09 电流监测标签维护（扫码/手输查询、新增、改绑、监测记录；见 [MOB-PWR-01](#mob-pwr-01-定稿2026-07-29)）
- [x] MOB-F-10 电流监测基站维护（查询/新增/修改、监测记录；见 [MOB-PWR-02](#mob-pwr-02-定稿2026-07-29)）
- [x] MOB-UI-02 App 首页底部三栏：消息 / 工作台 / 我的；个人资料页
- [ ] 其余建议见 [MOB.9](#mob9-移动端与小程序后续能力建议2026-07-21) / [MP.2](#mp2-小程序二期建议2026-07-21)，条目入第 7 章

**MOB-SHR-01 定稿（2026-07-28）**

| 项 | 定稿 |
|----|------|
| **范围** | App（meis-mobile）+ 小程序（meis-mp）；复用 Web `/shared/loan`、`/shared/return` |
| **能力** | 借调申请、借调审核、确认借出（lend）、归还申请、归还审核 |
| **选设备** | 公用设备列表（`is_shared_device` + 在库）+ 扫码（校验公用） |
| **权限** | 菜单码 `shared_loan` / `shared_loan_approve` / `shared_return` / `shared_return_approve`；首页按权限显隐 |
| **状态机** | 与 Web 一致：draft→pending→approved→lend→on_loan→归还 pending→returned |
| **范围外** | 收费、借调记录汇总、公用设备登记管理（继续 Web） |

**MOB-PWR-01 定稿（2026-07-29）**

| 项 | 定稿 |
|----|------|
| **范围** | App（meis-mobile）+ 小程序（meis-mp）；复用 `/api/power/tag/**`（meis-analytics） |
| **入口** | 首页「电流标签」；按菜单码 `power_tag` 显隐（本期复用 Web；独立移动权限码见 BACKLOG-MOB-14） |
| **查询** | 扫码 = 精确匹配 `tag_code`；手工输入 = 模糊匹配编码/名称；仅未软删；0 条→询问是否新增；多条→列表选中后再操作 |
| **新增** | 扫码未命中确认后进入新增，`tag_code` 预填且不可改；也可「直接新增」手工填编码（防条码无法识别） |
| **维护标签信息** | 可编：`tag_name`、所属基站、额定功率、安装日期、是否启用、备注；`tag_code` 新建后不可改 |
| **绑定设备** | 独立入口：查看当前绑定 / 改绑 / 解绑；改绑与解绑写 `power_tag_bind_log`；**一设备仅允许一个有效标签**（Q-03） |
| **监测记录** | 操作菜单「监测记录」；默认近 1 小时、逆序、可改时间/排序、分页；人工查看、不做正常标记（与 MOB-PWR-02 共用规则） |
| **途径** | `power_tag` 增 `create_channel` / `update_channel`；App/小程序/Web 写入口均写入；Web 列表/详情有列则展示 |
| **软删同码** | 查询视为不存在，可走新增；保存时按 `tag_code` 命中软删行则**恢复并更新**（与现网 `PowerTagController` 行为一致） |
| **失败提示** | 保存失败提示「请核对标签信息，可到 Web 端维护」 |
| **范围外** | 待机电流上下限、绑定历史查看（仍 Web）；实时采集 |

> 细则与验收见文末 [MOB-PWR-01](#mob-pwr-01-移动端电流监测标签维护定稿2026-07-29)。

**MOB-PWR-02 定稿（2026-07-29）**

| 项 | 定稿 |
|----|------|
| **范围** | App + 小程序；`/api/power/station/**` |
| **入口** | 首页「电流基站」；菜单码 `power_station` |
| **查询** | 列表 + 关键字（编码/名称/位置）；无扫码 |
| **维护** | 新增/修改：名称、院区、位置、IP、协议、状态、启用、备注；`station_code` 唯一且保存后不可改 |
| **监测记录** | 默认近 1 小时、逆序可切换、分页；人工确认安装/采集，系统不做标记 |
| **途径** | `power_base_station` 增 `create_channel` / `update_channel` |
| **软删同码** | 同标签：查询无、保存恢复 |
| **范围外** | 关联标签列表、标签↔基站改绑（见 BACKLOG-PWR-01） |

> 细则见文末 [MOB-PWR-02](#mob-pwr-02-移动端电流监测基站维护定稿2026-07-29)。
...
| Q-03 | 电流监测 | 一设备多标签是否允许 | 禁止：同一设备同时仅允许一个未软删且有效绑定的标签 | **已确认**（2026-07-29，MOB-PWR-01） |
...
| 2.217 | 2026-08-06 11:30:00 | — | REP-STAT-01：筛「已验收」含验收后结案单；列表/详情展示对齐为已验收 |
...
| 2.204 | 2026-08-01 12:00:00 | — | AST-SHEET-DRILL-01：执行单 Sheet 点单号下钻只读抽屉（头+明细+执行结果）；演示种子补结果行 |
...
| 2.197 | 2026-08-01 08:45:00 | — | AST-BF-01：历史补录保存→确认→生效后不可改删；评估编辑态各 Sheet 适用范围 |
| 2.196 | 2026-08-01 08:35:00 | — | 补强 OWN：Sheet 名「归属历史」、编辑「补录归属历史」；AST-LOC-01 位置真变更/纠错方案；约定包 §5.17 |
| 2.195 | 2026-08-01 08:25:00 | — | AST-OWN/DISP/UI-22/23、PLT-DEV-SHEET-01 方案定稿（不改业务码）；约定包 §5.16；BACKLOG 入池 |
...
| 2.189 | 2026-08-01 00:40:00 | — | 公共字段增验收/启用/安装/存放/责任人；AST-UI-20 卡片横纵向+导出；手写表时间列对齐 PLT-DT-01 |
| 2.188 | 2026-08-01 00:25:00 | — | PLT-DEV-LIST-01 补「公共字段清单」「公共筛选清单」两表（原「公共清单」仅指代码路径不易查找） |
...
| 2.178 | 2026-07-31 16:25:00 | — | PLT-STATUS-CN-01 修订：状态/字典才「未知(码)」；编码名称规格等属性原样展示 |
| 2.177 | 2026-07-31 16:16:00 | — | 弹窗色与品牌一致：`list-surface` 后于 global 加载；`applyBrandColor` 同步 `--meis-modal-*` |
| 2.176 | 2026-07-31 16:10:00 | — | PLT-UI-SURFACE-01：`AppModal variant=report`（色条标题/报表底栏）；院区新增编辑启用 |
| 2.175 | 2026-07-31 16:10:00 | — | AST-UI-17：验收日期筛选/列；编辑态电流监测 Tab（绑已有标签/待机上下限/读数） |
| 2.174 | 2026-07-31 15:53:00 | — | PLT-UI-SURFACE-01：外框圆角改回 2px（不要弧度） |
| 2.173 | 2026-07-31 15:50:00 | — | AST-UI-16：资产登记启用日期命名；电流标签/录入/生产日期筛选与列表列 |
| 2.172 | 2026-07-31 15:42:00 | — | PLT-UI-SURFACE-01：查询盒/表格外框圆角 2px→6px（略柔和） |
| 2.171 | 2026-07-31 15:40:00 | — | PLT-UI-SURFACE-01：院区管理改为价值结构表同款 DOM/CSS（去 el-card）；`list-surface.css` 与 ANA-UI-05 1:1 |
| 2.170 | 2026-07-31 15:25:00 | — | MOB-SCAN-04：App/小程序摄像头与扫码入口全量统一加固 |
| 2.169 | 2026-07-31 15:20:00 | — | PLT-UI-SURFACE-01：列表页表面样式试点院区管理；`list-surface.css` + `SystemPageCard variant=report` |
| 2.168 | 2026-07-31 15:20:00 | — | MOB-SCAN-03：扫码页 CameraX NPE（延迟 start + 关 Impeller + 中文错误） |
| 2.167 | 2026-07-31 15:00:00 | — | PLT-THEME-01：偏好「系统颜色」全局品牌色（预设+调色盘，localStorage）；分析报表走 `--meis-report-*` |
| 2.166 | 2026-07-31 12:10:00 | — | 脚本分区：`scripts/{common,bs,app}` + `package/{common,bs,app}`；根入口薄转发；约定包附录 A |
| 2.165 | 2026-07-31 10:30:00 | — | PLT-STATUS-CN-01 三端状态中文 + PLT-I18N-01 国际化架构预留；附录 R 修订；约定包 §5.1/§5.13 |
| 2.164 | 2026-07-30 15:40:00 | — | PKG-VCS-01：package/www 等构建产物移出版本库；.gitignore 加固 |
| 2.163 | 2026-07-30 14:55:00 | — | PLT-DT-01 扩至 App/小程序：统一日期时间工具与展示入参 |
| 2.162 | 2026-07-30 14:50:00 | — | PLT-DT-01：Web 日期/时间统一格式 + 查询控件入参；约定包 §5.12 |
| 2.161 | 2026-07-30 10:00:00 | — | 合并冲突：完整/更新打包 + PKG-WEB 前端 www 同包保留 |
| 2.160 | 2026-07-30 09:45:00 | — | package：完整打包.bat + 更新打包.bat（指纹增量 → update\） |
| 2.161 | 2026-07-29 16:25:00 | — | ANA-UI-04：资产使用率统计前端表样式（六列，不接后台） |
| 2.160 | 2026-07-29 11:25:00 | — | DASH-UI-08：新增设备与资产分布并排半宽 |
| 2.159 | 2026-07-29 11:20:00 | — | DASH-UI-10：设备状态改同心 KPI gauge；移除独立 Multiple KPI gauge |
| 2.158 | 2026-07-29 10:25:00 | — | DASH-UI-09：首页设备状态按固定资产字典中文展示，排除已退货 |
| 2.157 | 2026-07-28 23:15:00 | — | DASH-UI-08：资产分布迁至数据分析「新增设备」下方 |
| 2.156 | 2026-07-28 22:55:00 | — | DASH-UI-08：日常办公「交通方式」改为「资产分布」（按科室台数/占比，deviceByDept） |
| 2.155 | 2026-07-28 22:40:00 | — | DASH-UI-07：快捷入口视口改为 5 列×2 行（原 4×3，高度偏高） |
| 2.156 | 2026-07-30 14:45:00 | — | MOB-SCAN-02：App 相机/扫码入口全量排查；拍照统一 ensureCameraPermission |
| 2.155 | 2026-07-30 14:40:00 | — | MOB-SCAN-01：安卓扫码页先申请相机权限再开预览，修复黑屏感叹号 |
...
| 2.145 | 2026-07-29 10:25:00 | — | MOB-PWR-01 定稿：移动端电流标签扫码维护；Q-03 禁止一设备多标签；BACKLOG-MOB-14 |
| 2.144b | 2026-07-28 22:30:00 | — | DASH-UI-07：快捷入口 4×3 视口+自动/手动翻页；上限 12→45（修订 DASH-UI-06） |
| 2.144a | 2026-07-28 21:45:45 | — | DASH-UI-06：快捷入口用户自选（sys_user.preferences.quickEntryPaths，侧栏有效菜单，上限 12） |
| 2.144 | 2026-07-28 21:30:00 | — | AST-UI-15：资产登记全选查询结果/取消全选、批量修改、列表追加厂家供应商与到期列 |
| 2.143 | 2026-07-28 21:00:00 | — | AST-UI-14：资产登记编辑/查看 Tab 拆分；计划/PM/电流读数与绑定；修订附录 P.2 |
| 2.142 | 2026-07-28 | — | SYS-UI-01：登录页左侧 fit2cloud 同构分层立体插画；右侧表单不变 |
| 2.141 | 2026-07-28 17:25:00 | — | OPS.16.30：修改/明细修改/修改记录途径齐套 + 约定 §5.10.5 |
...
| 2.133 | 2026-07-27 14:25:00 | — | MOB-UI-01 定稿：全 App+小程序；现有品牌色；BACKLOG-MOB-13 可排期 |
| 2.132 | 2026-07-27 14:20:00 | — | BACKLOG-MOB-13：App UI/操作简洁化（范围待确认） |
...
| 2.122 | 2026-07-23 22:15:00 | — | OPS.16.27：途径展示齐套（制单/审核/执行/确认）+ 跨业务约定 |
| 2.121 | 2026-07-23 22:05:00 | — | OPS.16.26：Web/移动端执行明细展示执行途径、确认途径 |
...
| 2.114 | 2026-07-23 19:55:00 | — | OPS.16.19：Web 执行态去掉「开始执行」；直接明细执行/确认 |
| 2.113 | 2026-07-23 19:50:00 | — | OPS.16.18：执行单无驳回；查看无审核按钮；有执行记录禁删单/明细 |
| 2.112 | 2026-07-23 19:40:00 | — | OPS.16.17：未确认明细可重复执行/修改；确认后锁定 |
| 2.111 | 2026-07-23 18:10:00 | — | BACKLOG-OPS-01 / OPS.16.16 落地：纳入申请表+三端申请+Web 确认；移动端扫码四入口 |
| 2.110 | 2026-07-23 17:50:00 | — | OPS.16.16：现场漏设备先直开执行；纳入计划申请+Web确认；移动端执行四入口；BACKLOG-OPS-01 |
...
| BACKLOG-ANA-13 | 效益 | 费用手工登记完整业务（读写、人次/分摊规则、接台账、Excel 导入落库） | ANA-UI-12 布局+导入示意已挂 | P2 | 前端示意已落地；业务待排期 | 可排期 |
...
| BACKLOG-OPS-01 | 运维 | 漏设备：直开执行 + 纳入计划申请/Web确认；App/小程序执行四入口 | OPS.16.16 | P1 | — | 已完成 |
...
| BACKLOG-MOB-13 | 移动端 | App+小程序 UI/操作简洁化（全量；现有品牌色） | [MOB-UI-01](#mob-ui-01-app--小程序界面与操作简洁化待开发2026-07-27) | P2 | ①②③已落地 | 已完成 |
...
| BACKLOG-MP-06 | 小程序 | 执行明细可删除已拍照片（对齐 App） | MP.4 | P1 | 体验对齐 | 已完成 |
| BACKLOG-MP-07 | 小程序 | 报修表单对齐 App：故障图/类型/备注；服务端草稿+撤回 | MP.4 | P1 | 方案3全量 | 已完成 |
| BACKLOG-MP-08 | 小程序 | OPS 体验微调：到期 meta、纳入计划 picker 信息、「可修改」提示 | MP.4 | P2 | 体验对齐 | 已完成 |
| BACKLOG-MOB-14 | 移动端 | 移动端功能独立菜单权限码，与 Web 菜单码分离 | MOB-PWR-01 / 用户约定 | P1 | 全量移动功能统一改造 | 可排期 |
| BACKLOG-MOB-15 | 移动端 | 电流标签：待机电流上下限 / 绑定历史（移动端） | MOB-PWR-01 范围外 | P2 | 监测记录已并入 MOB-PWR-01/02 | 可排期 |
| BACKLOG-MP-09 | 小程序 | 台账详情档案附件 / UDI 变更史只读区 | MOB-GAP-01 | P2 | 本批证照/培训已做 | 可排期 |
| BACKLOG-MOB-16 | 移动端 | 本地台账同步 G1 标量；报废进度只读；档案/UDI 史 | MOB-GAP-01 | P2 | 扫码查台账本批已做 | 可排期 |
| BACKLOG-MOB-17 | 移动端 | 科室盘点申请/报表（执行已支持；申请归 Web） | G5/P-06 | P2 | 列表已标 check_type | 可排期 |
| BACKLOG-MOB-18 | 移动端 | 小程序首页 tabBar 对齐 App 三栏（消息/功能/我的） | MOB-UI-02 | P2 | App 已落地 | 可排期 |
| BACKLOG-PWR-01 | 电流监测 | 标签↔基站绑定策略（临近基站多站识别） | MOB-PWR-02 F | P1 | 业务规则待定 | 可排期 |
...
| 层级 | 说明 |
|------|------|
| `refSelectConfig.ts` | 定义外键表 → API、`labelKey`（名称）、`codeKey`、`showCode` |
| `formatRefRowLabel` / `useRefLabelMap.ts` | 预加载；`resolveRefLabel`；强业务默认拼「编码 名称」（§5.2.1） |
| `pageSchemas.ts` | `collectLinkTables`；字段级 `linkHideCode` 可只显名称 |
| `CrudPage` | 挂载时预加载主表 + 明细表全部外键，列表 `TableCellValue` 显示标签 |
| `DeviceDetailTabs` | 设备详情 Tab 预加载 `medical_device` 外键，复用 `TableCellValue` |
| `RefDisplay` / `RefSelect` | 只读/编辑外键；与标签图同一拼装规则 |

### H.3 验证

1. 资产管理 → 双击设备进入详情 →「基本信息」中生产厂商、供应商应显示名称而非 UUID；强业务外键宜见编码。
2. 各 CRUD 列表中外键列（科室、库房、设备等）应显示可读文本（强业务为「编码 名称」）。
3. 只读表单字段（如审批人）应显示姓名而非 ID。
4. 采购计划院区/科室下拉仍只显示名称（`linkHideCode`）。

### H.4 强业务拼编码（PLT-REF-CODE-01）

见 [PLT-REF-CODE-01](#plt-ref-code-01-强业务外键显示编码名称定稿2026-08-01)。
...
> **修订（AST-UI-14，2026-07-28）**：纠正编辑态漂移（曾误露出全部业务 Sheet）；查看态补计划 / PM / 电流读数与标签绑定。  
> **修订（AST-UI-17，2026-07-31）**：编辑/新增追加「电流监测」Tab（绑已有标签 / 待机上下限 / 监测记录）；查看态仍用原电流两 Tab。

| 模式 | 可见 Tab |
|------|----------|
| **新增** | 基本信息 + 设备档案 + 设备图片 + **电流监测** + **维保信息**（无设备 id 时不可绑定/维护） |
| **编辑** | 基本信息 + 设备档案 + 设备图片 + **电流监测** + **维保信息**（不含资产卡片与其余业务 Sheet） |
| **查看** | 见 [AST-UI-14](#ast-ui-14-定稿2026-07-28) 完整顺序；含保养/巡检/计量/PM 计划与记录、电流读数、电流标签绑定记录 |
...
**状态**：P.6 已实施（2026-07-12）；**P.2 修订按 AST-UI-14 实施（2026-07-28）**。
...
| 主题 | 位置 |
|------|------|
| **跨项目可复用约定全集** | [reusable-engineering-conventions.md](reusable-engineering-conventions.md)（**v1.48**） |
...
| 外键显示名称 / 强业务拼编码 | [附录 H](#附录-h外键字段显示名称2026-07-12)、[PLT-REF-CODE-01](#plt-ref-code-01-强业务外键显示编码名称定稿2026-08-01)、约定包 §5.2.1 |
...
| 列表字典中文 / I18N 预留 / 新状态种子 | [附录 R](#附录-r列表状态与字典值中文显示)、[PLT-STATUS-CN-01](#plt-status-cn-01-三端状态枚举中文展示定稿2026-07-31)（含 **PLT-DICT-SEED-01**）、[PLT-I18N-01](#plt-i18n-01-国际化架构预留定稿2026-07-31)、约定包 §5.1 / §5.13 |
...
| 修改记录途径 | [OPS.16.30](#ops1630-修改--明细修改--修改记录途径齐套已完成2026-07-28)、约定包 §5.10.5 |
...
| 从属设备列表字段/筛选约定 | [PLT-DEV-LIST-01](#plt-dev-list-01-定稿2026-07-31)（含[公共字段清单](#公共字段清单plt-dev-list-01)、[公共筛选清单](#公共筛选清单plt-dev-list-01)）、约定包 §5.15 |
| 台账单据/计划 Sheet 表头与下钻 / 生成方式 | [PLT-DEV-SHEET-01](#ast-own-01--ast-own-02--ast-loc-01--ast-disp-01--ast-ui-22--ast-ui-23--plt-dev-sheet-01-定稿2026-08-01)、[AST-SRC-01](#ast-src-01--plt-dev-sheet-01-生成方式列定稿2026-08-01)、约定包 §5.16 |
| 归属 / 补录确认 / 非维修换件 / 位置 / 处置（方案） | OWN、[BF-01](#ast-bf-01-台账历史补录确认生效定稿2026-08-01)、[PART-01](#ast-part-01-非维修配件更换定稿2026-08-01)、LOC、DISP；约定包 §5.17；第 7 章 BACKLOG |
| 三甲缺口再审（标量/对象/流程） | [AST-GAP-REVIEW-01](#ast-gap-review-01-定稿2026-08-01--三甲再审双部分--流程)、[专文](meis-requirements-tertiary-gap-review.md) |
...
### Q.15 2026-08-01 主数据详情业务 Sheet 表头与下钻

用户确认：台账等主数据查看态挂载的单据/计划类 Sheet，列表列对齐对应业务列表关键表头，状态中文，行可下钻至同模块详情并见最底层明细；无独立详情页时至少只读抽屉展示头+明细。

已写入 [PLT-DEV-SHEET-01](#ast-own-01--ast-own-02--ast-loc-01--ast-disp-01--ast-ui-22--ast-ui-23--plt-dev-sheet-01-定稿2026-08-01)、Q.6 索引、交付自检与 [约定包 v1.38 §5.16](reusable-engineering-conventions.md#516-主数据详情中的业务单据--计划-sheet)。同期定稿归属区间 / 处置聚合 / 改科室 / Sheet 状态中文（业务实现见第 7 章 BACKLOG，待开工）。

### Q.16 2026-08-01 台账归属历史补录与位置真变更/纠错

用户确认：

1. 编辑态增加「补录归属历史」Sheet，写入区间表后在「归属历史」可见；查看 Sheet 命名采用「归属历史」（优于「归属变更记录」）。
2. 楼层/房间等位置变更采用与科室相同的按钮弹窗：确定后选择「真变更（生成历史）」或「纠错（只改正台账）」。

已写入 AST-OWN-01/02、AST-LOC-01、[约定包 §5.17](reusable-engineering-conventions.md#517-主数据关键字段真变更与纠错)、`BACKLOG-AST-LOC-01`（实现 P2）。

### Q.17 2026-08-01 台账历史补录确认生效

用户确认：评估编辑态现有/规划 Sheet，对需要的补录操作增加「保存后待确认、确认后生效、生效后不可删改」。

已写入 [AST-BF-01](#ast-bf-01-台账历史补录确认生效定稿2026-08-01)：补录归属/位置历史采用；科室/位置按钮真变更与基本信息/档案/图片/电流/维保不采用本模式。约定包 §5.17 同步。

### Q.18 2026-08-01 非维修配件更换与生成方式列

用户确认：① 编辑态新增「非维修配件更换」Sheet，走确认生效；② 查看 Sheet 若同时含业务生成与手工补录，须在列表后部增加「生成方式」列区分。

已写入 [AST-PART-01](#ast-part-01-非维修配件更换定稿2026-08-01)、[AST-SRC-01](#ast-src-01--plt-dev-sheet-01-生成方式列定稿2026-08-01)、约定包 §5.16、`BACKLOG-AST-PART-01`。

### Q.19 2026-08-01 三甲缺口再审（标量 / 对象 / 流程）

用户确认：以三甲评审为标准复核字段与业务流程缺口；文档须分「标量字段」与「从表/独立对象（可含单独维护）」，并覆盖流程缺口；先文档后开发。

已写入 [AST-GAP-REVIEW-01](#ast-gap-review-01-定稿2026-08-01--三甲再审双部分--流程) 与专文 [`meis-requirements-tertiary-gap-review.md`](meis-requirements-tertiary-gap-review.md)。

---

## 附录 R：列表状态与字典值中文显示

> 来源：用户需求（2026-07-12）；修订 **PLT-STATUS-CN-01**（2026-07-31）：扩至三端 + 强制补字典 + I18N 预留（见 [PLT-I18N-01](#plt-i18n-01-国际化架构预留定稿2026-07-31)）。

### R.1 合理性结论

- **合理**：业务字典（如 `device_status=normal`）在库中已有中文标签，界面却直接渲染英文码，属于展示缺口。
- **范围**：Web / App / 小程序凡用户可见的状态/枚举字段；布尔 `is_*` 无字典时「是/否」（`is_active`/`is_clinical` 保持启用停用、临床/非临床）。
- **风险**：缺种子时旧逻辑会裸码直出 → 改为补齐 `R__data_fix` + 统一回退 `未知(码值)`；不改变存库英文码。

### R.2 实现要点

1. Web：`useDict` → 静态 catalog（`i18n/statusCatalog`）→ `未知(码)`；`StatusTag` / `TableCellValue` 必走此链。
2. App / 小程序：共享 `status_labels` / `statusLabels`，页面禁止各自维护不一致 map。
3. 列表进入时预加载本页 `dictType`；自定义页同等要求。
4. 新枚举必须先写字典种子再上线展示。

**状态**：R.2 按 PLT-STATUS-CN-01 实施。
...
### REP-STAT-01 已验收筛选与展示（定稿·2026-08-06）

> 来源：报修申请筛「已验收」无数据；验收通过后主状态立即 `closed`、子状态 `verified`。

| 项 | 定稿 |
|----|------|
| **根因** | 验收通过写 `status=closed` + `repair_sub_status=verified`；字典「已验收」码为 `verified`，纯 `status IN ('verified')` 永远 0 |
| **筛选** | 选 `verified` 时同时匹配：`status=verified` **或**（`status=closed` 且（`repair_sub_status=verified` 或 `verify_time` 非空）） |
| **展示** | 上述结案单在列表/详情将 `status` 展示为 `verified`（标签「已验收」）；跳过验收直接结案仍为「已关闭」 |
| **部署** | 重启/热加载 **meis-repair** |
...
- **变更记录表**（附录 T.4b，2026-07-22；**T.4d** 2026-07-28）：列 = 时间 / 动作 / 操作人 / **途径**（web/app/mp 中文，多端业务必显）/ **变更字段**（中文名，如「供应商」「规格」）/ **改动前** / **改动后** / 备注；一次修改多字段时按字段拆行展示；接口须把 `changed_fields` jsonb 解析为数组再返回。多端单据修改记录见 OPS.16.30。
...
| **执行列表列** | 执行单号、计划单号、执行类型、来源、模板、级别/类型、计划/执行日期、状态、执行人、开始/结束时间、制单途径、审核途径、制单人、制单时间、审核人、审核时间 |
| **约定** | 通用原则见约定包 **§5.9**（v1.21）；途径展示见 **§5.10** / OPS.16.27 |
...
| **执行** | 头表可编时打开结果弹窗；完成后 `completed`；**未确认前可再次修改**（OPS.16.17）；写执行人/`end_time`/`execution_channel` |
| **确认** | ~~仅 `completed`~~ → **修订见 OPS.16.15**：未完成亦可确认（自动记 completed）；写 `confirmed` + 途径 + `confirmed_by`/`_name`/`confirmed_at` |
| **查看** | 始终可打开明细 |
| **删除** | ~~仅未确认可软删~~ → **修订见 OPS.16.18**：无执行记录才可删；已有执行痕迹/已确认不可删；头表已提交/已审核不可删 |
...
#### OPS.16.16 漏设备补执行 · 纳入计划申请确认（2026-07-23）

> 来源：上线期台账不全场景 + 用户确认（1 先执行；2 纳入须申请+Web 确认，三端可发起；3/4 移动端不做新建计划；扫码后四能力）。

| 项 | 定稿 |
|----|------|
| **场景** | 计划/执行已生成后，现场发现设备未纳入；需先能做完本轮，再把设备写回计划覆盖 |
| **本轮执行** | **先直开执行即可**（`ad_hoc`，字段同 OPS.16.15）；与是否纳入计划**解耦**，可不申请纳入也照常做完 |
| **新建计划** | **Web 保留**既有计划新建；**App / 小程序不做**新建计划 |
| **纳入计划** | 三端（Web / App / 小程序）均可**发起纳入申请**；**仅 Web 计划详情确认**后才写入计划明细 |
| **自动建计划** | **本期不做**（不为每台漏设备自动生成计划单） |

##### 纳入申请 / 确认

| 项 | 定稿 |
|----|------|
| **目标计划** | 同模块、**已审核**、设备尚未在该计划明细中；申请人须选择目标计划（可按科室/模板筛选） |
| **申请内容** | `plan_id`、`device_id`（+code/name 快照）、申请人、申请途径、申请时间、备注；状态 `pending` / `approved` / `rejected` |
| **发起端** | Web / App / 小程序；途径写 `create_channel`（web/app/mp） |
| **确认端** | **仅 Web** 计划详情（或计划下「纳入申请」区）：通过 / 驳回（驳回须原因） |
| **通过效果** | 追加 `plan_item`（按计划周期算 `next_due_date`，刷新头表到期缓存）；**不**自动写入该计划下进行中执行单（本轮仍靠直开；下次「生成执行」才带上） |
| **驳回** | 状态 `rejected`，计划明细不变；可再次申请 |
| **约束** | 同一设备对同一计划仅允许一条 `pending`；已在明细中不可再申请 |
| **数据** | 建议共享表 `ops_plan_include_request`（`module`=maintain/inspect/pm）+ 姓名快照；进固定迁库槽位 |

##### 移动端执行界面（扫码 / 检索设备后）

| 能力 | 说明 |
|------|------|
| **新增执行单** | 直开 `ad_hoc`（模板/类型/周期/执行日期/起止时间，同 OPS.16.15） |
| **申请纳入计划** | 选目标已审核计划 → 提交纳入申请（待 Web 确认） |
| **执行明细** | 列出该设备未审核执行明细 → 填结果（对齐 OPS.16.14/16.15） |
| **确认明细** | 逐条确认（未填完也可确认，同 16.15） |

> App / 小程序 **不做**：新建计划、确认纳入申请（确认只在 Web）。

##### Web 生成计划字段（移动端不做新建；Web 勿漏）

| 类别 | 字段 |
|------|------|
| **必填** | 计划名称、模板、级别/类别（保养级别 / 巡检类别 / PM 类别）、周期类型+周期值（天数只读）、责任科室、责任人、设备明细≥1 |
| **建议** | 院区（保养/PM；巡检若无列可后续对齐）、备注 |
| **系统写** | 计划单号、制单人/时间、审核状态/人/时间、计划状态、途径 |
| **明细** | 设备 id+code/name、科室快照、下次到期（默认可按周期推算） |

**状态**：已定稿并落地（2026-07-23）；实现见第 7 章 **BACKLOG-OPS-01**（已完成）。

#### OPS.16.17 未确认可改执行 · 确认后锁定（2026-07-23）

> 来源：用户截图（已完成后仅见「确认」无法再改）+ 草稿确认。

| 项 | 定稿 |
|----|------|
| **范围** | 保养 / 巡检 / PM 执行明细（Web 执行态；App/小程序对齐） |
| **未确认** | 无论 `pending` / `in_progress` / `completed`，均可再次打开结果并**修改**后保存（操作文案：未完成显示「执行」，已完成显示「修改」） |
| **已确认** | 不可再改结果/再执行；仅可「查看」；删除亦不可（同 16.14） |
| **后端** | `complete` / `patch item` 仅在 `status=confirmed` 时拒绝；`completed` 允许覆盖写 |

**状态**：已定稿并落地（2026-07-23）。

#### OPS.16.18 执行单无驳回 · 查看无审核 · 有执行记录禁删（2026-07-23）

> 来源：用户截图（列表审核列「通过/驳回」、查看底栏「审核通过/驳回」）+ 草稿确认。

| 项 | 定稿 |
|----|------|
| **范围** | 保养 / 巡检 / PM 执行单（Web；后端三模块对齐） |
| **驳回** | **取消**执行单驳回能力；`POST …/audit` 的 `action=reject` 返回 400「执行单不支持驳回」；列表审核列仅「通过」 |
| **查看** | 查看态底栏**不**展示「审核通过 / 驳回」；审核仅列表「通过」入口 |
| **有执行记录** | 明细满足任一即算：`status ∈ {completed,confirmed}`，或 `end_time` / `executor_id` 非空，或 `overall_result` 非空 |
| **删明细** | 有执行记录不可删（含已确认）；仅无痕迹且头表可编时可软删 |
| **删单据** | 草稿且**无一**明细有执行记录才可删；`DELETE …/execution/{id}`；Domain 删同步拦截 |
| **列表字段** | `page` 增加 `has_execution_record` 供删除按钮显隐 |

**状态**：已定稿并落地（2026-07-23）。

#### OPS.16.19 执行态去掉「开始执行」（2026-07-23）

> 来源：用户截图（执行界面底栏「开始执行」）+ 草稿确认。

| 项 | 定稿 |
|----|------|
| **范围** | 保养 / 巡检 / PM Web 执行态（`openMode=execute`） |
| **UI** | 底栏**不**展示「开始执行」；打开执行后直接对明细「执行 / 确认」 |
| **后端** | `POST …/start` 保留（兼容/移动端）；Web 不再调用 |

**状态**：已定稿并落地（2026-07-23）。

#### OPS.16.20 纳入申请：可申请计划列表 · 多选批量（2026-07-23）

> 来源：用户截图（申请纳入仅下拉选计划）+ 草稿确认。

| 项 | 定稿 |
|----|------|
| **范围** | 保养 / 巡检 / PM：设备管理「申请纳入计划」（Web）；App/小程序列表对齐过滤 |
| **列表口径** | 仅**已审核**计划；且对该设备：**明细中尚无** + **无 pending 纳入申请**（驳回后可再申请） |
| **多设备** | 计划出现条件：所选设备中**至少一台**可申请；多设备时列「可纳台数」 |
| **展示字段** | 计划单号、名称、模板、级别/类别、责任科室、责任人、周期、下次到期、明细数、状态、制单人等 |
| **查询条件** | 关键词（单号/名称/模板/科室/责任人）、责任科室、模板、计划状态、下次到期区间 |
| **交互** | 计划表格**多选**；一次提交可对多个计划批量建申请；备注共用 |
| **API** | `GET …/include-request/approved-plans?device_ids=&keyword=&dept_id=&template_id=&status=&next_due_from=&next_due_to=`；`POST …/include-request/batch`（`plan_ids`×`device_ids`，跳过不可申请组合） |

**状态**：已定稿并落地（2026-07-23）。

#### OPS.16.21 查看态纯只读 · 计划/执行列表查询条件（2026-07-23）

> 来源：用户截图（查看保养计划仍见生成/补录/纳入通过驳回）+ 草稿确认。

| 项 | 定稿 |
|----|------|
| **范围** | 保养 / 巡检 / PM：计划详情「查看」、执行详情「查看」；列表筛选 |
| **计划查看** | **仅只读**：隐藏生成执行单、执行补录、勾选批量生成/补录、行内生成/补录、纳入「发起/通过/驳回」、激活/审核等变更入口；明细日期不可改；列表「生成/补录」列仍保留（非查看态入口） |
| **执行查看** | 底栏仅「关闭」；不展示撤回等变更按钮 |
| **计划列表筛** | 审核状态、计划状态、责任科室、模板、下次到期区间；关键词含单号/名称/模板/责任人 |
| **执行列表筛** | 状态、来源、执行类型、制单途径、计划/执行日期区间；关键词含执行单号/计划单号/模板/执行人 |

**状态**：已定稿并落地（2026-07-23）。

#### OPS.16.22 App/小程序执行对齐 Web（方案 A）（2026-07-23）

> 来源：用户要求按 Web 架构补齐 App/小程序保养·巡检·PM 执行缺口；确认方案 **A**（现场壳对齐，不做完整执行列表）。

| 项 | 定稿 |
|----|------|
| **定位** | App / 小程序继续做**现场扫码执行壳**；执行列表查看/编辑/审核通过/删单**仅 Web** |
| **直开** | 与 OPS.16.15 一致：模板、级别/类别、`cycle_type`/`cycle_value`/`cycle_days`、`planned_date`、起止时间（默认当天 00:00:00～23:59:59；选模板可回填周期） |
| **明细锁定** | 对齐 OPS.16.17：头表可编且 `item.status≠confirmed` 才可改；已完成文案「保存修改」；执行入口不列已确认明细 |
...
#### OPS.16.26 执行明细展示执行/确认途径（2026-07-23）

> 来源：用户草稿（Web + 移动端明细显示确认途径、执行途径）。

| 项 | 定稿 |
|----|------|
| **字段** | 明细已有 `execution_channel`（完成时写）、`confirm_channel`（确认时写）；字典 `execution_channel`：web/app/mp |
| **Web** | 保养/巡检/PM 执行单详情「设备明细」表增加「执行途径」「确认途径」列（中文标签） |
| **App/小程序** | 执行明细页头部展示「执行途径」「确认途径」 |
| **空值** | 未执行/未确认时显示「—」 |

**状态**：已落地（2026-07-23）；**由 OPS.16.27 补齐头表制单/审核途径与跨业务约定**。

#### OPS.16.27 途径字段齐套展示 · 跨业务约定（2026-07-23）

> 来源：用户草稿（制单/审核/确认/执行途径都要显示；其他业务也要如此）。

| 项 | 定稿 |
|----|------|
| **头表（执行单）** | 列表与详情须展示 **制单途径** `create_channel`、**审核途径** `audit_channel`（中文；空为「—」）；详情在「执行」态亦常显（不因隐藏头表单而丢失） |
| **明细** | 继续展示 **执行途径**、**确认途径**（同 OPS.16.26） |
| **App/小程序** | 明细页同时展示头表制单/审核 + 明细执行/确认 |
| **跨业务约定** | 凡库表已有 `*_channel`（取值 web/app/mp）的单据，列表/详情**必须**展示对应中文途径；有字段则显，无字段不强加。当前：运维执行三模块 + 纳入申请 `create_channel` |
| **可复用包** | 同步写入约定包「途径展示」 |

**状态**：已落地（2026-07-23）。

#### OPS.16.28 执行审核人/时间 · 纳入申请途径（2026-07-23）

> 来源：用户草稿（执行界面看不到审核人/审核时间；纳入计划申请途径与确认途径要显示）。

| 项 | 定稿 |
|----|------|
| **执行列表** | 三模块执行列表增加 **审核时间** `audited_at`（审核人已有） |
| **执行详情/执行态** | 头表摘要常显：制单途径、审核途径、**审核人**、**审核时间**（空为「—」；Web 执行弹窗 + App/小程序明细页） |
| **纳入申请** | `create_channel` 展示为 **申请途径**；新增 `confirm_channel`（通过/驳回时写），展示为 **确认途径** |
| **库表** | `ops_plan_include_request.confirm_channel` 进 R__/V1 |
...
### MP.4 对照近期 App/OPS 的小程序缺口审计（2026-07-23）

> 来源：用户草稿（鉴于 App 修改优化，检查小程序是否需修补）。

#### 已对齐（本期无需再补）

| 项 | 说明 |
|----|------|
| 设备搜索 | 编码/名称/首拼 lookup（`/repair/workorder/devices/lookup`） |
| OPS.16.25 | 直开级别/类型主数据下拉（三模块配置一致） |
| OPS.16.26/27 | 制单/审核/执行/确认途径中文展示 |
| OPS.16.28 | 审核人、审核时间 |
| 扫码执行 / 四入口 / 完成·确认 / 签名 | 流程与 App 一致 |
| 我的报修进度 + 验收 | MOB.10 / MP.2 已落地 |
| 工程师维修 / 本地离线草稿 / 计量 / 补打 | **故意 App 专属**，小程序不做 |

#### 方案3已落地（2026-07-23）

用户确认全量对齐 App 报修流（含草稿+撤回）+ OPS 体验。

| 项 | 落地 |
|----|------|
| **执行删图** | `ops/detail.vue` 可编辑态角标删除检查项/设备项照片 |
| **报修表单** | 故障类型、备注、故障图≤3；保存草稿后询问是否提交 |
| **草稿/撤回** | 「我的报修」分「申请单 / 进度验收」；申请单支持编辑草稿、提交、撤回、删除 |
| **OPS hub** | 到期补设备编码；纳入计划 picker 信息加厚；已完成明细标「可修改」；到期点入带 `dept_id` |
| **不做** | 本地离线草稿仍为 App 专属 |
...
### MOB-UI-01 App / 小程序界面与操作简洁化（已完成·2026-07-27）

> 来源：美化 UI 与用户操作，采用更简洁清爽的布局。记入第 7 章 `BACKLOG-MOB-13`。

| 项 | 定稿 |
|----|------|
| **目标** | 视觉与操作更简洁清爽：减少拥挤卡片/多层嵌套，统一间距与主操作层级 |
| **端** | **Flutter App（`meis-mobile`）+ 微信小程序（`meis-mp`）同步** |
| **页面范围** | **全量**（非仅高频页） |
| **视觉** | **沿用现有品牌色 `#1677FF`**；不做新色板/换肤；轻量整理对比度与层次 |
| **原则** | 单页一事；主 CTA 醒目；列表密度适中；少装饰色块与多级阴影；**不**借机改业务流程与后端契约 |
| **实施建议** | 分批：① 主题/壳层与共用组件 ② 高频页（扫码报修、我的报修、运维 hub、盘点）③ 其余页与小程序对齐 |
| **待开发编号** | `BACKLOG-MOB-13` |
| **不做** | 换品牌色；为美化改接口；单端先做另一端长期不同步 |

#### 落地进度

| 批 | 范围 | 状态 | 说明 |
|----|------|------|------|
| ① | 主题/壳层 + 首页（双端） | **已落地** | App：`app_colors` / `app_spacing` / `app_theme`、`MeisNavTile` / `MeisSectionLabel`、首页分区导航；MP：`uni.scss` 令牌、首页/登录简洁化、全局白底导航栏 |
| ② | 高频业务页（扫码报修、我的报修、运维 hub、盘点等） | **已落地** | App：`MeisListCard` / `MeisStatusChip`；报修列表/表单、我的报修、工程师 inbox、运维 hub（单主 FAB）、盘点、计量 hub；MP：报修扫码/我的报修、运维 hub 对齐令牌与细边卡片（MP 无盘点页） |
| ③ | 其余页面对齐 | **已落地** | App：消息、登录、标签补打、盘点/报修/工程师/运维/计量详情、签名板；MP：资产扫码/列表/详情、报修详情、运维详情/签名、不良事件上报 |

**状态**：已完成（`BACKLOG-MOB-13`；①②③均已落地）。

### MOB-UI-02 App 首页底部三栏导航（2026-08-03）

> 来源：首页底部增加「消息 / 功能 / 我的」；现有功能入口放入「功能」。

| 项 | 定稿 |
|----|------|
| **端** | 本期 **Flutter App（`meis-mobile`）**；小程序 tabBar 对齐见 BACKLOG-MOB-16 |
| **底栏** | 三栏固定：**消息** · **工作台** · **我的**；默认落在 **工作台** |
| **消息** | 原消息中心列表（已读/跳转逻辑不变）；不再在功能列表重复入口 |
| **工作台** | 业务入口列表（常用功能 / 运维执行等）；**不含**品牌问候头 |
| **我的** | 微信式资料头（点击进 **个人资料**）；分组列表：修改密码 / 设置 IP / 关于 / 退出；无「你好」问候语 |
| **个人资料** | 头像、账号、租户、名字、性别、地区、手机号、微信号；`GET /api/system/users/me/profile`；库表 `sys_user.gender/region/wechat_id` |
| **实现** | `MainShellPage` + `ProfilePage`；改密 `POST .../me/change-password`；设置 IP 回填本地配置 |
| **不做** | 本期不改小程序；不改消息后端契约 |
...
### OPS.16.30 修改 / 明细修改 / 修改记录途径齐套（已完成·2026-07-28）

> 来源：修改、明细修改、修改记录等也要补全途径，并添加到约定。扩展 OPS.16.29。

| 项 | 定稿 |
|----|------|
| **头表修改** | 写 `update_channel`（最后一次修改端）；列表/详情展示 |
| **明细修改** | 明细表有 `update_channel` 时，patch/完成前改写均覆盖写入；展示于明细列或摘要 |
| **单据修改记录** | `sys_doc_change_log.client` 写入归一后的 web/app/mp；修改记录 UI 展示中文「途径」 |
| **实体变更记录** | 多端单据（如报修）`sys_entity_change_log.client` 同步写入并展示；纯 Web 主数据可空（默认 web） |
| **约定** | 约定包 §5.10.5 + §4.4 |

**状态**：已完成。

### AST-UI-14 资产登记编辑/查看 Tab 齐套（已完成·2026-07-28）

> 来源：用户草稿 + A–E 确认；修订附录 P.2。定稿见 [§3 AST-UI-14](#ast-ui-14-定稿2026-07-28)。

**状态**：已完成。

### AST-UI-15 资产登记列表全选/批量修改/追加列（已完成·2026-07-28）

> 来源：用户草稿 + A–D 确认。定稿见 [§3 AST-UI-15](#ast-ui-15-定稿2026-07-28)。

**状态**：已完成。

### MOB-SHR-01 移动端公用设备借调（已完成·2026-07-28）

> 来源：用户草稿 + A–E 确认。定稿见 [§3.19 MOB-SHR-01](#mob-shr-01-定稿2026-07-28)。

**状态**：已完成。

### MOB-PWR-01 移动端电流监测标签维护（定稿·2026-07-29）

> 来源：用户草稿 + A–G 确认；衍生：途径字段、移动权限码分离入池。摘要见 [§3.19 MOB-PWR-01](#mob-pwr-01-定稿2026-07-29)。

#### 1. 范围与端

| 项 | 定稿 |
|----|------|
| **端** | App（meis-mobile）+ 小程序（meis-mp） |
| **后端** | meis-analytics `/api/power/tag/**`；与 Web 标签维护同一数据模型 |
| **权限** | 本期首页入口按 Web 菜单码 `power_tag` 显隐；后续独立移动权限码见 **BACKLOG-MOB-14** |
| **入口名** | 「电流标签」 |
...
| 路径 | 行为 |
|------|------|
| **扫码** | 识别内容为 **标签编码 `tag_code`**（精确匹配）；非标签码提示请扫描电流标签条码 |
| **手工输入** | 按编码/名称 **模糊查询**；仅 `is_deleted` 未删 |
| **0 条** | 提示是否新增；确认后进入新增（扫码场景预填 `tag_code` 且锁定） |
| **1 条** | 直接进入操作页 |
| **多条** | 列表展示（编码、名称、设备编码/名称、基站、启用），用户选中后再操作 |
| **软删同码** | 查询视为不存在；可走新增流程；保存时命中软删行则 **恢复并更新** |

#### 3. 操作

| 操作 | 说明 |
|------|------|
| **维护标签信息** | 可编：`tag_name`、所属基站、额定功率、安装日期、是否启用、备注。`tag_code`：扫码新增锁定；直接新增可手工填写；**保存后不可改** |
| **直接新增** | 不经扫码，手工填 `tag_code`（防条码无法识别） |
| **查看绑定设备** | 独立入口；展示当前 `device_id` 对应编码/名称等；未绑定明示 |
| **修改绑定设备** | 独立入口；选设备改绑或 **解绑**；变更须写 `power_tag_bind_log` |
| **一设备一标签** | 禁止同一设备被多个未软删标签同时绑定（**PWR-B-03 / Q-03**）；Web 同步校验 |
| **监测记录** | 操作菜单入口；规则见 [MOB-PWR-02 §3](#3-监测记录基站--标签共用规则)（默认近 1 小时、逆序、可筛可排序、分页；人工查看） |

#### 4. 途径与数据

| 项 | 定稿 |
|----|------|
| **列** | `power_tag.create_channel`、`power_tag.update_channel`（`web`/`app`/`mp`）；固定迁库槽位补列 |
| **写** | 新建写 `create_channel`；任意保存/改绑/解绑写 `update_channel`；Web 保存同步写 |
| **显** | Web 标签列表/详情有列则展示中文途径；空为「—」 |
| **失败** | 业务/校验失败提示：**请核对标签信息，可到 Web 端维护信息** |

#### 5. 本期不做

- 待机电流上下限、绑定历史查看（Web 已有；见 **BACKLOG-MOB-15**）
- 移动端独立菜单权限码全量改造（**BACKLOG-MOB-14**）
- 实时采集 / MQTT

#### 6. 验收要点

- [ ] App / 小程序均可扫码精确查、手输模糊查；多条可选
- [ ] 未命中可新增；扫码新增 `tag_code` 锁定；直接新增可手填编码
- [ ] 维护信息与改绑/解绑成功；改绑写绑定日志
- [ ] 设备已被其他标签占用时拒绝并提示
- [ ] 软删同码：查询无、保存恢复并更新
- [ ] `create_channel` / `update_channel` 写入正确；Web 可展示
- [ ] 无 `power_tag` 权限时入口隐藏
- [ ] 可查看标签监测记录（默认近 1 小时、逆序、分页）

#### 7. 落地说明（2026-07-29）

- 后端：GET /api/power/tag/by-code/{tagCode}；保存写 create_channel/update_channel；PWR-B-03 一设备一标签
- 迁库：R__columns_biz.sql 补 power_tag 途径列（需租户 Flyway/Ensuring 执行）
- App：首页「电流标签」；power_tag_hub/form/bind
- 小程序：pages/power/hub|form|bind
- Web：途径列展示；保存传 client=web；编辑锁定编码
- **MOB-PWR-02**：补监测记录页（App/小程序）

**状态**：已完成（监测记录随 MOB-PWR-02 补齐）。

### MOB-PWR-02 移动端电流监测基站维护（定稿·2026-07-29）

> 来源：用户草稿 + A–H 确认；标签侧监测记录并入 MOB-PWR-01。摘要见 [§3.19 MOB-PWR-02](#mob-pwr-02-定稿2026-07-29)。
...
|----|------|
| **端** | App（meis-mobile）+ 小程序（meis-mp） |
| **后端** | meis-analytics `/api/power/station/**`；读数复用 `/api/power/station/{id}/readings/page` |
| **权限** | 首页入口按 Web 菜单码 `power_station` 显隐；独立移动权限码见 BACKLOG-MOB-14 |
| **入口名** | 「电流基站」 |
| **无扫码** | 基站无条码/二维码；列表 + 关键字查询 |

#### 2. 基站维护

| 项 | 定稿 |
|----|------|
| **查询** | 关键字模糊匹配编码/名称/位置；仅未软删 |
| **新增 / 修改** | 可编：`station_name`、院区、安装位置、IP、协议类型、状态、启用、备注；`station_code` 唯一，**保存后不可改** |
| **软删同码** | 查询视为不存在可新建；保存命中软删行则**恢复并更新** |
| **途径** | `power_base_station.create_channel` / `update_channel`；App/小程序/Web 写入口均写入；Web 有列则展示 |
| **范围外（本期）** | 关联标签列表、标签↔基站改绑（临近基站可能多站识别，见 BACKLOG-PWR-01） |

#### 3. 监测记录（基站 + 标签共用规则）

> 目的：现场工人确认安装/采集是否正常；**人工查看**，系统不做「正常/异常」标记。

| 项 | 定稿 |
|----|------|
| **入口** | 基站：列表/详情 →「监测记录」；标签：电流标签操作菜单 →「监测记录」（修订 MOB-PWR-01） |
| **默认窗口** | 打开时默认 **最近 1 小时**（`read_at`）；可改开始/结束时间后查询 |
| **排序** | 默认读取时间 **逆序**；可切换正序/逆序 |
| **分页** | 需要 |
| **导出** | 本期不做 |
| **列（至少）** | 标签编码、设备编码、电流(mA)、读取时间、插入时间；基站视角可显基站编码 |

#### 4. 验收要点

- [ ] App / 小程序可查询、新增、修改基站；编码保存后不可改
- [ ] 软删同码：查询无、保存恢复
- [ ] 基站/标签均可查看监测记录；默认近 1 小时、逆序、可改时间与排序、分页
- [ ] `create_channel` / `update_channel` 写入；Web 可展示
- [ ] 无 `power_station` / `power_tag` 权限时对应入口隐藏

#### 5. 落地说明（2026-07-29）

- 后端：基站保存写途径、编码锁定、软删同码恢复；读数 API 已有
- 迁库：`R__columns_biz.sql` 补 `power_base_station` 途径列
- App：电流基站 hub/form；共用 `PowerReadingsPage`；标签操作补监测记录
...
### MOB-CHANNEL-02 历史途径回填 + 展示齐套 + 约定 §5.10.6（定稿·2026-07-29）

> 来源：前端途径是否齐；存量 `*_channel` 多为 NULL（早前 Web）；追加多端须自动加列并回填历史为 web。

#### 1. Web 展示

- 巡检/保养/PM 执行头表、纳入申请：`delete_channel` 改为列表可见（`list: true`）
- 其余多端业务途径列此前已 `list: true`（报修/盘点/计量/不良/借还/电流等）

#### 2. 历史回填（`R__data_fix`）

对当前 schema 下全部 `*_channel` 列：

| 规则 | 说明 |
|------|------|
| `create_channel` | NULL → `web` |
| `update` / `submit` / `confirm` / `audit` / `execution` / `delete` | **有动作证据**才 → `web` |
| 未发生动作 | 保持 NULL，UI「—」 |

#### 3. 约定沉淀

- 约定包 **v1.26 §5.10.4 / §5.10.6**：Web→多端追加时必须加列 + 历史回填 + 三端展示
- 双写本条；后续同类需求按约定执行，无需再口头提醒

#### 4. 验收

- [ ] 重启租户迁库后，老单据制单途径多为 Web，未提交单据提交途径仍为空
- [ ] 运维执行列表可见删除途径列（软删行有值）
...
### PLT-REF-CODE-01 强业务外键显示「编码 名称」（定稿·2026-08-01）

> 来源：列表/对象/主表明细涉及厂家、供应商、科室等强统计字段时，名称同时显示编码，便于复制到筛选或主数据菜单搜索。

| 项 | 定稿 |
|----|------|
| **适用** | 生产厂家、供应商、科室、院区、仓库、楼宇、设备/资产/财务分类 |
| **不适用** | 人员（姓名）、单位字典 |
| **格式** | `编码 名称` |
| **范围** | Web 列表单元格、详情、表单/筛选下拉（`refSelectConfig` + `useRefLabelMap`） |
| **例外** | **采购计划**院区/科室（及申报人）继续 `linkHideCode` 只显名称 |
| **约定** | 双写约定包 **§5.2.1** v1.47；附录 H.4 |

**验收**

- [ ] 资产台账列表科室/厂家/供应商等外键列为「编码 名称」
- [ ] 供应商/仓库下拉可见编码
- [ ] 采购计划院区/科室下拉仍仅名称
- [ ] 人员字段仍仅姓名
...
### MOB-SYNC-01 多端功能演进同批（定稿·2026-08-01）

> 来源：修改 Web 与移动端都有的功能时，若字段扩展或业务分支扩展，须同时改移动端，避免事后补洞。

| 项 | 定稿 |
|----|------|
| **触发** | 业务能力已在 Web **与** App/小程序共有；本次为字段扩展或业务分支/动作扩展 |
| **强制** | 同一交付覆盖后端 + Web + **已有入口的 App 与/或小程序**；禁止只改 Web |
| **自检** | 对照该业务移动端页面/接口清单，字段展示与操作入口已跟上 |
| **例外** | 纯 Web 专属；或经确认写入第 7 章 `BACKLOG-*` 并点名「移动端待跟」 |
| **约定** | 双写约定包 **§5.18** v1.48；与 §5.10 途径约定并行 |

**验收**

- [ ] 后续多端共有功能扩展时，交付说明中点名 App/MP 改动或 BACKLOG
- [ ] 交付自检含「移动端是否同批」项（见交付规则）

**状态**：已完成（约定沉淀；后续交付按此执行）。

### AST-UI-25 查看态侧栏内容滚动（定稿·2026-08-01）

> 来源：资产登记-查看：优化导航栏和内容显示；内容溢出覆盖了滚动条。

| 项 | 定稿 |
|----|------|
| **问题** | 顶栏 Tab 全宽负 margin 误套到侧栏 `FormTabNav`，右侧内容撑破盖住滚动条 |
| **规则** | 顶栏专用样式仅 `--top`；侧栏与内容区各自 `min-width/min-height:0` + 独立纵向滚动；内容区 `scrollbar-gutter: stable` 并留足右内边距 |
| **范围** | 资产登记查看态侧栏布局；编辑态顶栏不变 |
| **约定** | 双写约定包 **§5.19** v1.49 |

**验收**

- [ ] 查看态左侧 Sheet 可独立滚动，无底边线/负 margin 拉通误伤
- [ ] 右侧字段区滚动时滚动条完整可见，内容不压条
- [ ] 编辑态顶栏 Tab 全宽底边仍正常
...
### MOB-SCAN-01 安卓扫码相机无预览（定稿·2026-07-30）

> 来源：安卓安装 App 后扫码调用摄像头无响应（黑屏+感叹号）。截图页标题「扫码锁定设备」。

#### 1. 根因

| 项 | 说明 |
|----|------|
| 现象 | `RepairScanPage` 黑屏中央白色感叹号 = `mobile_scanner` **默认 errorBuilder** |
| 原因 | 扫码页**未先申请运行时相机权限**就创建 `MobileScannerController`；运维/盘点等入口也未先 `Permission.camera.request` |
| 清单 | `AndroidManifest` 已有 `CAMERA`；缺的是**进入页后的运行时授权与错误提示** |

#### 2. 定稿修补

| 项 | 做法 |
|----|------|
| 扫码页 | 进入后先 `Permission.camera.request`，**授权成功后再**创建控制器并展示预览 |
| 拒绝 | 明确文案 +「重试」；永久拒绝提供「打开系统设置」；从设置返回自动重试 |
| 相机错误 | 自定义 `errorBuilder`（勿只显示黑屏感叹号） |
| Manifest | 补充 `camera.autofocus` feature（required=false） |

#### 3. 验收

- [ ] 首次进入扫码弹出系统相机授权；允许后出现预览
- [ ] 拒绝后有文案与重试/去设置，不再静默黑屏
- [ ] 报修/运维/盘点/电流标签/公用借调扫码入口均可用（共用 `RepairScanPage`）
...
| 入口 | 能力 | 原状 | 处理 |
|------|------|------|------|
| `RepairScanPage` | 扫码预览 | MOB-SCAN-01 已修：页内申请权限后再开相机 | 保持；**所有扫码统一进此页** |
| 报修表单扫码 | → RepairScanPage | 外层曾申请权限 | 去掉重复申请，交给扫码页 |
| 运维 Hub 扫码 | → RepairScanPage | **原先未申请** | 依赖扫码页 |
| 盘点明细扫码 | → RepairScanPage | **原先未申请** | 依赖扫码页 |
| 电流标签 Hub/改绑扫码 | → RepairScanPage | 外层曾申请 | 去掉重复，交给扫码页 |
| 公用借调扫码 | → RepairScanPage | 外层曾申请 | 去掉重复，交给扫码页 |
| 报修故障拍照 | ImagePicker 相机 | 拒绝时**静默** return | `ensureCameraPermission` + 提示/去设置 |
| 运维执行拍照 | ImagePicker 相机 | 拒绝时**静默** return | 同上 |
| 电流基站 | 无扫码 | — | 无需改 |
| 相册选图 | gallery | 未强制相机权限 | 本期不动（系统相册选择器） |

#### 2. 约定

- **扫码**：只走 `RepairScanPage`（页内管权限与错误 UI）
- **拍照**：统一 `ensureCameraPermission`（`lib/shared/utils/camera_permission.dart`）
- 禁止在未授权时直接创建 `MobileScannerController` / 调起相机

#### 3. 验收

- [ ] 运维/盘点扫码首次弹出授权并出预览
- [ ] 报修/运维拍照拒绝时有提示，永久拒绝可「去设置」
- [ ] 各扫码入口行为一致
...
### PKG-VCS-01 现场包构建产物移出版本库（定稿·2026-07-30）

> 来源：`package\www` 为前端生产构建产物，不需要版本控制；并排查其他同类文件。

#### 1. 定稿

| 路径 | 处理 |
|------|------|
| **`package/www/`** | 移出索引；仅保留 `package/www/.gitignore`（`*` + `!.gitignore`，同 `jars`/`apk`）；根 `.gitignore` 双保险 |
| **`meis-web/dist/`** | 已有 `dist/` ignore，仍被跟踪的残留文件一并移出索引 |
| **`package/update/`** / **`package/logs/`** | ignore（增量交付包 / 运行日志） |
| **`package/env.txt`** | 移出索引并 ignore（含本机路径与口令）；只提交 `env.example.txt` |
| **仓库根 `tmp-*.txt` / `tmp-*.json`** | 移出索引并 ignore（调试临时文件） |

- 本地磁盘文件保留；需要前端静态资源时执行 `package\完整打包.bat` / `pack.ps1` 重新生成 `www\`。
- **以后不再上传**：依赖 `.gitignore`；勿再 `git add -f package/www/**`。

#### 2. 验收

- [ ] `git ls-files package/www` 仅见 `.gitignore`
- [ ] 完整打包后 `www` 有文件但不出现在 `git status` 待提交列表
- [ ] `package/env.txt` 本地可改且不进版本库

**状态**：已完成。

### PLT-STATUS-CN-01 三端状态/枚举中文展示（定稿·2026-07-31）

> 来源：用户需求（状态英文直出 → 中文）；确认：全三端；未命中时**强制补齐字典**（现阶段尽量）；衍生热点（运维执行/采购/打印/报表/健康检查）一并纳入。

#### 1. 定稿

| 项 | 约定 |
|----|------|
| **存库** | 状态/枚举列继续存英文码（snake_case） |
| **展示** | Web / App / 小程序列表、详情、标签、打印等**用户可见处**一律中文标签 |
| **主路径** | `sys_dict`（`dict_type` + `dict_value/code` → `dict_label`） |
| **缺字典** | 本期尽量在 `R__data_fix.sql` **补齐种子**；展示层静态目录仅作离线/未加载兜底，须与种子一致 |
| **未命中回退** | **仅**状态/枚举/分类字典字段：禁止裸码直出，显示 `未知(码值)`，并应尽快补字典 |
| **适用范围** | 加固对象 = **状态类**（`*_status`/`state` 等）与 **分类/下拉字典类**（schema 带 `dictType`）；**不含**对象业务属性（编码、名称、规格、型号、序列号等） |
| **属性字段** | 编码/名称/规格/型号等按实际值原样展示；英文或数字很正常，**禁止**套「未知(…)」 |
| **自定义页** | 状态/字典列不得绕过统一解析（报表/追溯/计量/大屏/选择器/打印等同规）；属性列勿误调 `resolveCodedLabel` |

#### 2. 落地

| 端 | 入口 |
|----|------|
| Web | `meis-web/src/i18n/*` + `useDict` / `StatusTag` / `TableCellValue`；schema 补 `dictType`；`isStatusField` 词段匹配防误伤 |
| App | `meis-mobile/lib/shared/utils/status_labels.dart`（调用方显式传 dictType，勿扫全字段） |
| 小程序 | `meis-mp/src/utils/statusLabels.ts`（同上） |
| 种子 | `meis-tenant/.../R__data_fix.sql` 补齐缺失 `*_status` 字典 |

修订附录 **R**；约定包 **§5.1 / §5.13**。

> **修订（2026-07-31）**：规避「全单元格走状态解析」——标签维护等页曾把编码/名称打成「未知(码)」；已收紧 `TableCellValue` / `formatStatusLabel` / `isStatusField`。

#### 3. 验收

- [x] Web CRUD 与自定义热点页状态列为中文（无 `draft`/`pending`/`in_use` 等裸码）
- [x] App / 小程序报修、借还、盘点、资产、运维执行详情状态为中文
- [x] 运维执行头/明细状态字典齐全且三端一致
- [x] 平台服务健康 `UP`/`DOWN` 显示「正常/异常」（或等价中文）
- [x] 属性列（编码/名称/规格/型号等）原样展示，不出现「未知(属性值)」

**状态**：已完成（含属性字段误伤修复）。

### PLT-I18N-01 国际化架构预留（定稿·2026-07-31）

> 确认：本期**不做**运行时语言切换；须为**本仓库内 I18N 升级**预留架构（极大概率原地改造，拆分代码库仅作兜底）。

#### 1. 架构原则（强制）

| 原则 | 说明 |
|------|------|
| **码值与文案分离** | DB/API 永存稳定码；用户文案在「标签层」解析 |
| **单一解析入口** | 各端集中 `resolveCodedLabel` / `statusLabels`；页面禁止散落硬编码中英文字典（特例页面须注明并回写目录） |
| **默认 locale** | 当前固定 `zh-CN`；解析 API 预留 `locale` 参数，后续可接文案包或 `sys_dict` 多语言列 |
| **字典即配置源** | `sys_dict` 为业务码首选标签源；静态 catalog 与种子双轨对齐，便于日后抽成 `locales/*.json` |
| **UI 壳文案** | 菜单/按钮等可暂硬编码中文，但新增功能应避免把业务状态写死在组件内 |
| **后端** | 不引入 MessageSource 除非切语言；可选 `*_label` enrich 仅方便移动端，码值字段仍保留 |

#### 2. 后续升级路径（预留）

1. Web：`vue-i18n`（或等价）挂载同一 key 空间；Element Plus locale 已有中文基础。  
2. App：`flutter gen-l10n` / ARB，与字典 key 对齐。  
3. 小程序：启用 uni-i18n，与 Web 共享 JSON。  
4. 可选：`sys_dict` 增加 `locale` 或独立 `sys_dict_i18n` 表。  
5. **优先本仓库改造**；仅当框架冲突无法解决时再考虑从本套代码生成平行产品线。

#### 3. 新增功能门禁

- 新状态机 / 枚举：必须先落 `sys_dict` 种子，再挂 schema `dictType` / 三端解析。  
- Code Review：用户可见状态不得直接 `{{ row.status }}` / `'${status}'` 裸绑。
...
> 已定稿并实现：**AST-UI-16**、**AST-UI-17**（见上文资产登记清单）。

**FAQ（AST-UI-17）**：编辑态应在「设备图片」后出现 Tab「电流监测」。若仍只见 3 个 Tab，多为浏览器/现场包仍加载旧前端：硬刷新（Ctrl+F5）或重启 `meis-web` 开发服务；若走 Nginx/`package/.../www` 静态包，需重新 `npm run build` 并覆盖 www。源码：`DeviceLedgerForm.vue` → `ledgerKeys` 含 `power_monitor`，面板 `DevicePowerMonitorPanel.vue`。

> **PLT-STATUS-CN-01 修订（v2.178）**：标签维护等页编码/名称被打成「未知(…)」已修复——仅状态与 `dictType` 分类字段走中文/`未知(码)`；编码、规格、型号等属性原样显示。


> 已定稿并实现：**AST-WRN-02**；**AST-UI-18～23**；**AST-GAP-01**；**AST-DEMO-01**；**PLT-DEV-LIST-01**；**PLT-DB-SYNC-01**；**PLT-DEV-SHEET-01**；**AST-OWN-01/02**；**AST-BF-01**；**AST-PART-01**；**AST-SRC-01**；**AST-LOC-01**（主路径）；**AST-DISP-01**。
> **部署**：重启 **meis-tenant** 以建 `device_ownership_period` / `device_part_replacement` / `device_location_period` 并灌字典；再重启业务服务与前端。
...
> **AST-SHEET-DRILL-01（2026-08-01）**：台账 Sheet 点单号/「明细」→ 只读抽屉（头+明细+执行结果）。种子：data/seed/demo_asset_ledger.sql（含 *_execution_result）。部署后重跑种子并硬刷新前端。

> **AST-STATUS-CN-GAP-01（2026-08-01）**：扫描代码 dictType 与 R__data_fix / statusCatalog.zh-CN.ts 对照后补齐：
> - 审批：maintain_approval_status / inspect_approval_status / metrology_approval_status（及 withdrawn/rejected）
> - 结果：inspect_result、pm_exec_result、pm_exec_status；计量补 qualified/unqualified/conditional
> - 其它：disposition_biz_status、sync_status、udi_change_reason；处置 Sheet 的 biz_status、UDI 史 change_reason 改用正确 dictType
> **部署**：重启 **meis-tenant** 灌字典 → 重启前端；已有库依赖可重复执行的 R__data_fix。

> **PLT-DICT-SEED-01（2026-08-01）**：继续开发若产生新状态/枚举码，须同步 R__data_dict + 三端 catalog（约定包 §5.1 v1.43）。与 PLT-STATUS-CN-01 / AST-STATUS-CN-GAP-01 配套。

> **MOB-GAP-01（2026-08-01）**：对照 Web G1～G5，移动端最小落地——
> - **小程序**：台账详情补 G1 标量（UDI/责任人/类别等）+ 证照/培训授权只读
> - **App**：工作台「台账查询」：顶部搜索条件 + 下方资产列表（`/asset/device/page`），点选进轻量详情（G1 标量 + 证照/培训只读）；盘点列表/详情展示 check_type（科室盘点）与科室名；双端 statusLabels 对齐
> - **入池**：档案/UDI 史 BACKLOG-MP-09；本地 G1 同步/报废只读 BACKLOG-MOB-16；科室盘点申请 BACKLOG-MOB-17；报废全流程/外部处置仍归 Web
> **验证**：重新编译 App / 小程序；打开扫码查台账与盘点任务页。

> **AST-DEMO-01 修订（2026-08-01）**：演示种子覆盖列表前三页设备（默认每页 20 → `SD24080001`～`060`）各业务 Sheet，不再仅样板机一条。部署：`SET search_path TO tenant_demo;` 后重跑 `data/seed/demo_asset_ledger.sql`。
...
> **PLT-REF-CODE-01（2026-08-01，v1.47）**：强业务主数据外键列表/详情/下拉显示「编码 名称」（厂家/供应商/科室/院区/仓库/楼宇/三类分类）；人员/单位只显名称；**采购计划**院区/科室继续只显名称。刷新前端即可。

> **MOB-SYNC-01（2026-08-01，v1.48）**：Web 与移动端**已共有**的功能，若字段扩展或业务分支扩展，须同批改 App/小程序（已有入口的端），禁止只改 Web 回头再补；暂缓须入 BACKLOG 并点名。约定包 §5.18。

> **AST-UI-25（2026-08-01，v1.49）查看态侧栏/内容滚动**：
> - **问题**：顶栏 Tab 的全宽负 margin 误套到侧栏导航，内容区撑破盖住滚动条
> - **规则**：顶栏专用样式只作用于 `--top`；侧栏与内容区各自 `min-width/min-height:0` + 独立 `overflow-y`；内容区 `scrollbar-gutter: stable` 并留足右内边距
...
> **REP-STAT-01（2026-08-06）**：筛「已验收」含验收后结案（closed+子状态 verified）；列表展示对齐。重启 meis-repair。
