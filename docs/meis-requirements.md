...
| BACKLOG-MP-06 | 小程序 | 执行明细可删除已拍照片（对齐 App） | MP.4 | P1 | 体验对齐 | 可排期 |
| BACKLOG-MP-07 | 小程序 | 报修表单对齐 App：故障图/类型/备注；可选服务端草稿+撤回 | MP.4 | P1 | 待产品确认范围 | 可排期 |
| BACKLOG-MP-08 | 小程序 | OPS 体验微调：到期 meta、纳入计划 picker 信息、「可修改」提示 | MP.4 | P2 | 体验对齐 | 可排期 |
...
| 优先级 | 项 | 文件 | BACKLOG |
|--------|----|------|---------|
| P1 | 执行明细可删除已拍照片 | `meis-mp/src/pages/ops/detail.vue` | BACKLOG-MP-06 |
| P1 | 报修补故障图片（建议）；可选故障类型、备注 | `meis-mp/src/pages/repair/scan.vue` | BACKLOG-MP-07 |
| P1/取舍 | 报修服务端草稿+撤回（App 有；MP 现为一键创建并提交） | 扩展 `scan`/`mine` | BACKLOG-MP-07 |
| P2 | 到期列表补设备编码；纳入计划 picker 信息加厚；已完成明细标「可修改」 | `hub.vue` | BACKLOG-MP-08 |

#### 不纳入本期

- 工程师台、本地离线草稿、计量移动、标签补打蓝牙：保持 App-only。
- 纳入申请途径列表：仅 Web 展示，移动端只提交。

**状态**：审计完成（2026-07-23）；修补项待用户确认范围后再开发。