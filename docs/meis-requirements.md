...
| **新增** | 基本信息 + 设备档案 + 设备图片 |
| **编辑** | 基本信息 + 设备档案 + 设备图片（**不含**资产卡片与右侧业务 Sheet） |
...
| 版本 | 日期 | 作者 | 变更说明 |
|------|------|------|----------|
| 2.176 | 2026-07-31 16:30:00 | — | PLT-UI-SURFACE-01 全站：SystemPageCard/AppModal/FormDrawer 默认 report；PageFilterBar 纳入查询色条 |
| 2.175 | 2026-07-31 16:16:00 | — | 弹窗色与品牌一致：`list-surface` 后于 global 加载；`applyBrandColor` 同步 `--meis-modal-*` |
| 2.174 | 2026-07-31 16:10:00 | — | PLT-UI-SURFACE-01：`AppModal variant=report`（色条标题/报表底栏）；院区新增编辑启用 |
| 2.173 | 2026-07-31 15:53:00 | — | PLT-UI-SURFACE-01：外框圆角改回 2px（不要弧度） |
...
| 2.170 | 2026-07-31 15:20:00 | — | PLT-UI-SURFACE-01：列表页表面样式（查询条件色条+报表风表格）试点院区管理；`list-surface.css` + `SystemPageCard variant=report` |
| 2.169 | 2026-07-31 15:00:00 | — | PLT-THEME-01：偏好「系统颜色」全局品牌色（预设+调色盘，localStorage）；分析报表走 `--meis-report-*` |
...
|------|----------|
| **新增** | 基本信息 + 设备档案 + 设备图片 |
| **编辑** | 基本信息 + 设备档案 + 设备图片（不含资产卡片与业务 Sheet） |
...
| 资产登记编辑/查看 Tab | [AST-UI-14](#ast-ui-14-定稿2026-07-28)、[附录 P.2](#p2-编辑-vs-查看-tab) |
...
| **未命中回退** | 禁止裸码直出；显示 `未知(码值)`，并应尽快补字典 |
| **自定义页** | 不得绕过统一解析（报表/追溯/计量/大屏/选择器/打印等同规） |

#### 2. 落地

| 端 | 入口 |
|----|------|
| Web | `meis-web/src/i18n/*` + `useDict` / `StatusTag` / `TableCellValue`；schema 补 `dictType` |
| App | `meis-mobile/lib/shared/utils/status_labels.dart` |
| 小程序 | `meis-mp/src/utils/statusLabels.ts` |
...
- [ ] Web CRUD 与自定义热点页状态列为中文（无 `draft`/`pending`/`in_use` 等裸码）
- [ ] App / 小程序报修、借还、盘点、资产、运维执行详情状态为中文
- [ ] 运维执行头/明细状态字典齐全且三端一致
- [ ] 平台服务健康 `UP`/`DOWN` 显示「正常/异常」（或等价中文）

**状态**：已完成（核心链路与热点页；残留裸码按 `未知(码)` 暴露并继续补字典）。
