...
| 主题 | 位置 |
|------|------|
| **跨项目可复用约定全集** | [reusable-engineering-conventions.md](reusable-engineering-conventions.md)（**v1.10**） |
...
### MOB.7 本地 SQLite 工具类（2026-07-21）
...
| **用法边界** | 缓存「当前任务/待同步队列」；**禁止**把全院主数据镜像进本地；大批量仍依赖服务端分页/增量 API |
| **衍生** | 盘点离线扫码队列、报修草稿本地暂存可基于本工具类排期（未做则记 BACKLOG） |
