...
| **样式** | 对齐 ANA-UI-05：浅蓝表头、隔行白/`#fef5e7`、悬停 `#e6f7ff`、表格线 `#d0d7de`；明细框撑满剩余高度；底留白约 16px |
...
| **明细底栏** | 合计行：总数量、总金额、当前页面数量、当前页面金额；底栏背景与「查询条件」表头一致 `#d6ebf8`；分页可选每页 **10 / 20 / 30 / 50** 条；底栏下留白与右侧一致（约 16px）；明细框撑满剩余高度 |
