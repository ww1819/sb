...
| **明细底栏** | 合计行：总数量、总金额、当前页面数量、当前页面金额；底栏背景与「查询条件」表头一致 `#d6ebf8`；分页可选每页 **10 / 20 / 30 / 50** 条；明细/汇总查询区重置后 **导出**（`价值结构分析表-明细表/汇总表YYYYMMDD.xls`，含加粗合计行） |
| **表上说明** | 明细/汇总表上方标题行（无背景）；「注：…」为**红色**；「查询条件」标题条背景 `#d6ebf8`；表头 `#d6ebf8`；隔行白/`#fef5e7`；悬停高亮 `#e6f7ff`；表格线浅灰 `#d0d7de` |
| **汇总查询** | 设备类型、净值率≥；查询 / 重置 |
...
| 2.171 | 2026-07-31 15:50:00 | — | AST-UI-16：资产登记启用日期命名；电流标签/录入/生产日期筛选与列表列 |
