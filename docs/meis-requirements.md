...
**AST-UI-21 定稿（2026-08-01）**

| 项 | 定稿 |
|----|------|
| **范围** | 资产登记 **查看态** Tab「配件更换记录」（在「维修记录」后） |
| **数据** | 读 `repair_workorder_segment_part`（`device_id`）联查配件档案/供应商/工单/进程类型 |
| **API** | `GET /api/asset/device/{id}/spare-replacements` |
| **不做** | 本期不新建更换表；非维修换件见 AST-GAP-01 衍生 |
...
| **查看 Tab 顺序** | 基本信息 → 资产卡片 → 设备档案 → 设备图片 → 资产标签 → 维修记录 → **配件更换记录**（AST-UI-21）→ 保养记录 → **保养计划** → 巡检记录 → **巡检计划** → 计量记录 → **计量计划** → **PM维护记录** → **PM维护计划** → 借调记录 → 借调费用 → 盘点记录 → 不良事件 → **电流读数** → **电流标签绑定记录** |
...
| BACKLOG-AST-GAP-01 | 资产 | 台账主表/从表缺口按 AST-GAP-01 分批加列与对象 | [缺口分析](meis-requirements-asset-ledger-gap.md) | P1 | UDI/责任人用户化优先 | 可排期 |
