...
| 主题 | 位置 |
|------|------|
| **跨项目可复用约定全集** | [reusable-engineering-conventions.md](reusable-engineering-conventions.md)（**v1.19**） |
...
| **提交确认（双阶段）** | 点「提交」→ 弹窗「是否确认提交？」；**取消**：无任何变更；**确定**：校验**全部明细已 `completed`** → 全部置 `confirmed`（写 `confirm_channel`）→ 头表 `submitted`（写 `submit_channel`） |
| **撤回** | 头表回 `in_progress`；明细 `confirmed` → `completed`（可再改后再提交确认） |
| **审核门槛** | **通过/驳回**前：全部明细须 `confirmed`，否则拒绝；审核写 `audit_channel` |
...
| **详情布局** | 标签**左置**（非置顶）；表单 `size=small`；主信息区 **4 列**网格；分组：**基本信息** / **执行与途径** / **制单审核**；收紧分组间距 |
