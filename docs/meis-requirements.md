...
| **引用招标计划** | 表单头「关闭」左侧；弹窗多选议价通过明细；确认后**整表替换**设备明细 |
| **弹窗列** | 勾选、设备名称、设备规格型号、数量、单价(`estimated_price`)、总金额(`total_price`) |
...
| **列表列** | 序号、单号、设备名称、申请科室、订单号、计划单号、规格型号、数量、总金额（`total_price`）；见 PUR-UI-20 |
| **工具栏** | 查询 / 重置 / 导出 |
| **操作列** | 见 PUR-UI-15（固定右侧「招标」） |
