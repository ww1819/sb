...
> 来源：用户草稿与确认（原文件末草稿）；一期仅 **扫码报修 + 双端故障图片**；盘点/保养扫码仍 backlog（MOB-F-02 等）。
...
| 纳入 | 不纳入（backlog） |
|------|-------------------|
| 扫码报修、编码锁定、草稿/提交/撤回/删除 | 盘点扫码 MOB-F-02 |
| Web + 移动端故障图片（S.6） | 保养/巡检扫码、复杂码制（URL/JSON） |
