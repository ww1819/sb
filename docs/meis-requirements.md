...
- [x] INS-UI-01 / MET-UI-01 运维下「巡检管理」「计量管理」；INS-UI-01b：巡检改挂保养下
...
**菜单结构（MT-UI-01 / INS-UI-01b，2026-07-17）**：运维管理 → **保养管理**（`ops_maintain`）→ 保养参数设置 / 保养计划 / 保养执行 / 保养记录查询；其下再挂 **巡检管理**（`ops_inspect`）→ 巡检参数设置 / 巡检计划 / 巡检执行 / 巡检记录查询（四级叶子）。路径不变。

**需求摘要**：

- [x] MT-UI-01 运维下增加二级「保养管理」，四项三级菜单挂其下
- [x] INS-UI-01b 巡检管理改挂保养管理下（嵌套 group），四项为四级菜单
...
> 运维二级分组终态（自上而下）：维修管理 → 保养管理（含巡检） → 计量管理 → 电流监测。
...
**菜单结构（INS-UI-01b，2026-07-17）**：运维管理 → 保养管理 → **巡检管理**（`ops_inspect`，`parent=ops_maintain`）→ 巡检参数设置 / 巡检计划 / 巡检执行 / 巡检记录查询。

**需求摘要**：

- [x] INS-UI-01 原：运维下二级「巡检管理」
- [x] INS-UI-01b 修订：巡检管理改挂保养管理下
...
| **菜单** | 可恢复保养/巡检/PM（及计量若需要）「设备管理」为 **过滤列表页**：仅展示已出现在对应模块计划明细或执行明细中的台账设备 |
