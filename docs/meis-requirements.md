...
| 主题 | 位置 |
|------|------|
| **跨项目可复用约定全集** | [reusable-engineering-conventions.md](reusable-engineering-conventions.md)（**v1.5**） |
...
### W.5 工作人员姓名冗余（2026-07-15）

> 来源：用户确认——审计列 + 业务责任人均冗余姓名；优先 `real_name`，空则 `username`；写入快照、不级联改名。  
> 可复用原则双写 [约定包 v1.5 §6.3](reusable-engineering-conventions.md)。
...
**状态**：W.5.2 + 维修 W.5.3 P0 本轮实施；其余业务责任人见 `BACKLOG-PLT-W02`。