...
| 项 | 定稿 |
|----|------|
| **位置** | 管理工作台数据分析区：设备状态 \| **设备分类** \| 国产/进口（同行三列，各 `span=8`） |
| **数据** | `/api/analytics/dashboard` 增加 `deviceCategory`：按 `medical_device.category_id` → `medical_device_category.category_name` 聚合台账数量；无分类记「未分类」；按数量降序取 TOP 8 |
...
| **匹配** | `GET /api/system/medical_device_category/match-by-device-name?name=`：`category_name ILIKE %名称%`；优先精确相等，再取名称更长、层级更深的叶子 |
