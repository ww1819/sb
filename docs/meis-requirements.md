...
- [x] MOB-UI-02 App 首页底部三栏：消息 / 功能 / 我的（现有入口归入「功能」）
...
| **我的** | 顶部 **MEIS / 问候语** + 账号卡片；操作：**修改密码** / **设置 IP** / **关于** / **退出登录** |
| **实现** | `MainShellPage` + `IndexedStack`；路由仍 `/home`；改密 `POST /api/system/user/me/change-password`；设置 IP 复用本地服务器配置 |
| **不做** | 本期不改小程序；不改消息后端契约 |
...
> - **App**：首页「扫码查台账」轻量详情（同上）；盘点列表/详情展示 check_type（科室盘点）与科室名；双端 statusLabels 对齐
