...
|------|------|------|----------|
<<<<<<< HEAD
| 2.73 | 2026-07-22 17:45:00 | — | 附录 T.4c：变更记录改动前/后明文（是否/字典/外键名）；跳过 extension_data |
| 2.72 | 2026-07-22 17:40:00 | — | 附录 T.4b：变更记录展示字段名+改动前/后；修复 jsonb 未解析导致空白 |
| 2.71 | 2026-07-22 17:30:00 | — | 附录 T.4：查看态变更记录按钮移至底部关闭旁（CrudPage/FormDrawer） |
| 2.70 | 2026-07-22 17:00:00 | — | AST-UI-13：资产登记合同/基本/折旧字段重排；去掉上次计量日期表单 |
=======
| 2.74 | 2026-07-22 17:55:00 | — | W.6.4：存量冗余缺口扫描（库房/维修/计量/盘点等）；充实 BACKLOG-PLT-W03 |
| 2.73 | 2026-07-22 17:50:00 | — | 附录 W.6 / Q.14：单据头与主数据冗余到明细/子表；约定包 v1.14 §6.2 |
| 2.72 | 2026-07-22 17:45:00 | — | OPS.14 扩展：计量计划编号 JL- / 执行单号 JX- 系统生成、禁止编辑 |
| 2.71 | 2026-07-22 17:40:00 | — | OPS.14：保养/巡检/PM 计划单号与执行单号系统生成、禁止编辑；约定包 v1.13 §6.8 |
| 2.70 | 2026-07-22 17:30:00 | — | OPS.13：保养/巡检/PM 设备管理列表对齐台账字段 + 状态/到期窗口/更多检索 |
>>>>>>> 2076fe694d62dddb4b0ff206b0f36d957b8faa66
...
| BACKLOG-PLT-W03 | 跨模块 | 存量主从单据明细缺业务单号/主数据 code·name 的分批补齐（明细见 W.6.4） | 附录 W.6.4 | P1–P2 | 新表已强制；P1 库房/维修/计量/盘点优先 | 可排期 |
...
| 主题 | 位置 |
|------|------|
| **跨项目可复用约定全集** | [reusable-engineering-conventions.md](reusable-engineering-conventions.md)（**v1.12**） |
...
**硬缺口（纳入 BACKLOG-PLT-W03，按批修补）**

| 优先级 | 模块 | 表 | 缺口 |
|--------|------|----|------|
| **P1** | 库房 | `device_entry_item` | 缺 `entry_no`；有 `device_name` **缺 `device_code`** |
| **P1** | 库房 | `device_outbound_item` | 缺 `outbound_no`（设备 code/name 已有） |
| **P1** | 库房 | `device_return_item` | 缺 `return_no`（设备 code/name 已有） |
| **P1** | 库房 | `device_goods_return_item` | 缺 `return_no`（设备 code/name 已有） |
| **P1** | 资产 | `inventory_check_item` | 缺 `check_no`（设备冗余已有） |
| **P1** | 维修 | `repair_workorder_event` / `process` / `segment` / `segment_part` | 设备已冗余；**缺 `wo_no`** |
| **P1** | 维修 | `spare_part_usage` | 设备已冗余；**缺 `wo_no`** |
| **P1** | 计量 | `metrology_execution_item` | 设备已冗余；**缺 `execution_no`** |
| **P1** | 公用 | `shared_device_fee` | **缺 `loan_no`**；设备冗余见 `BACKLOG-AST-W01` |
| **P2** | 采购 | `purchase_contract_item` | 缺 `contract_code` |
| **P2** | 采购 | `purchase_acceptance_item` | 缺 `acceptance_no`（若表在用） |
| **P2** | 运维内容行 | `*_execution_result`（保养/巡检/PM/计量） | 缺 `execution_no`（可经明细 join；非强制） |
| **P2** | 维修 | `repair_workorder_segment_user` | 缺 `wo_no`（人维度，可选） |
| **P2** | 巡检 | `inspection_record` | 有 `device_id` **缺 code/name** |
| **弱** | 运维计划/执行明细 | 上述 plan/execution item | 仅缺 `dept_name`（有 `dept_id` 时建议补） |

**说明**

- 调拨 `asset_transfer`、报废等为**单设备主单**无明细表，主单自身有单号+设备即可。
- 模板配置子表（`*_template_item`）不要求业务单号。
- 本轮只扫描建档，不自动迁库；实施时走固定槽位 + 写点同事务回填 + 可选历史回填。

**状态**：扫描完成（2026-07-22）；修补排期见第 7 章 `BACKLOG-PLT-W03`。
