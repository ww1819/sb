...
| Q-04 | 库房 | 资产模块与库房模块菜单重复，如何分工 | 入库/出库/调拨/盘点/报废入口统一在「库房管理」；资产台账侧对应菜单 `is_active=false`（保留路径与 API） | **已确认**（2026-07-15） |
...
| 主题 | 位置 |
|------|------|
| **跨项目可复用约定全集** | [reusable-engineering-conventions.md](reusable-engineering-conventions.md)（**v1.3**） |
...
| 维修调度/进程/工程师/验收 | [附录 U](#附录-u维修调度工程师进程与验收2026-07-13)（含 U.13） |
...
| **进程工程师** | 必选；默认 `assigned_user_id` 或当前用户；提交写段 `user_id`；无负责人时同步指派 |
| **开始时间** | 必填，默认当前时刻 |
...
检查一下全部代码，是否还有类似问题。包含insert和update。
添加维修进程：维修工程师默认接单人禁止编辑，添加修改勾选框。勾选之后可以修改维修工程师，要支持多选，因为一次进程可能多个工程师同时参与