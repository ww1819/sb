...
- [x] INS-UI-01 / MET-UI-01 运维下二级「巡检管理」「计量管理」，各自挂四项三级菜单
...
**菜单结构（MT-UI-01，2026-07-16）**：运维管理 → **保养管理**（`ops_maintain`，`menu_type=group`，侧栏为**可折叠二级子菜单**，非灰色分组标题）→ 保养参数设置 / 保养计划 / 保养执行 / 保养记录查询。路径不变。
...
**菜单结构（INS-UI-01，2026-07-16）**：运维管理 → **巡检管理**（`ops_inspect`，可折叠二级）→ 巡检参数设置 / 巡检计划 / 巡检执行 / 巡检记录查询。

**需求摘要（待补充）**：

- [x] INS-UI-01 运维下二级「巡检管理」，四项三级菜单挂其下
