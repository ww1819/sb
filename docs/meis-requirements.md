...
| **明细列** | 序号、设备编号、资产名称、规格/型号、单位、使用科室、月折旧率(%)、月折旧金额（元）、原值（元）、累计折旧金额（元）、净值（元）、购入日期、折旧科室、折旧比例、已折旧月份、存放地点；列宽须保证金额等字段完整显示不截断 |
| **汇总查询** | 使用科室；查询 / 重置 / 导出 |
| **汇总列** | 序号、使用科室、资产数量、原值合计、月折旧合计、累计折旧合计、净值合计 |
| **导出** | `折旧明细比例-明细表/汇总表YYYYMMDD.xls`，含加粗合计行 |
| **横向滚动** | 明细多列时，横向滚动条挂在**合计栏上沿**（表格滚动区底边），不得被合计/分页底栏遮挡；滚动条加粗便于拖动 |
