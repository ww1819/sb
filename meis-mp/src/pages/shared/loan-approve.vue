<template>
  <view class="page">
    ...
    <view v-else-if="!rows.length" class="empty">暂无待审核借调单</view>

    <view v-for="r in rows" :key="String(r.id)" class="card">
      <view>
        <text class="no">{{ r.loan_no || '—' }}</text>
        ...
        <text v-if="r.reason" class="desc">原因：{{ r.reason }}</text>
      </view>
...
async function approve(r: Record<string, unknown>) {
  const ok = await new Promise<boolean>((resolve) => {
    uni.showModal({
      title: '通过借调',
      content: `确认通过「${r.loan_no || ''}」？`,
      success: (res) => resolve(!!res.confirm)
    })
  })
  if (!ok) return
  try {
    await http.post(`/shared/loan/${r.id}/approve`, { client: 'mp' })
...
async function reject(r: Record<string, unknown>) {
  const ok = await new Promise<boolean>((resolve) => {
    uni.showModal({
      title: '驳回借调',
      content: `确认驳回「${r.loan_no || ''}」？`,
      success: (res) => resolve(!!res.confirm)
    })
  })
  if (!ok) return
  try {
    await http.post(`/shared/loan/${r.id}/reject`, { client: 'mp' })
